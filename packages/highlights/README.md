...
`Uint8Array` containing a `<pre class="highlights">` fragment. Byte input is
UTF-8. The bytes may reference WebAssembly memory and are valid until the next
highlighting or tokenization call on the same highlighter. Decode them
immediately, or call `html.slice()` to retain them.
...
Colors accept `#rgb`, `#rgba`, `#rrggbb`, `#rrggbbaa`, and numeric
`color(display-p3 r g b / alpha)` values (alpha is optional). Syntax scopes
inherit colors from the nearest dot-separated parent, then use the foreground
for font-only styles. `editor.background` falls back to `background`;
