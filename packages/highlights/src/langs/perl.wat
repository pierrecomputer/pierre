(module
  ...
  (keyword-table $perlWords $mem.perlWords $mem.powershellWords
    (group ;; 1: control
      "do" "if" "for" "else" "last" "next" "redo" "goto" "until" "elsif" "while" "unless" "return"
      "foreach")
    ...
    (group ;; 7: built-in functions
      "lc" "uc" "die" "map" "pop" "ref" "chop" "each" "eval" "exit" "grep" "join" "keys" "open"
      "push" "sort" "warn" "bless" "chomp" "close" "defined" "delete" "exists" "index" "print"
      "shift" "split" "length" "printf" "scalar" "substr" "values" "unshift" "sprintf" "wantarray")
      ...
      (then
        ...
        (if
          (i32.and
            ...
            (i32.eq (i32.load8_u (local.get $lhs)) (i32.const "x")))
  ...
  (func $perlQuoteKind (param $lhs i32) (param $rhs i32) (result i32)
    ...
    (if (i32.eq (local.get $n) (i32.const 1))
      (then
        ...
        (if (i32.eq (local.get $w) (i32.const "q"))
          (then (return (i32.const 1))))
        (if (i32.eq (local.get $w) (i32.const "m"))
    ...
    (if (i32.eq (local.get $n) (i32.const 2))
      (then
        (if (i32.eq (local.get $w) (i32.const "qw"))
          (then (return (i32.const 1))))
        (if (i32.eq (local.get $w) (i32.const "qq"))
          (then (return (i32.const 2))))
        (if (i32.eq (local.get $w) (i32.const "qr"))
          (then (return (i32.const 3))))
        (if (i32.eq (local.get $w) (i32.const "tr"))
          (then (return (i32.const 4))))))
    (i32.const 0))
  ...
  (func $perlQuoteBody
    ...
    (block $done
      (loop $scan
        ...
        (if
          (i32.or
            (i32.ge_u (global.get $ptr) (local.get $stop))
            (i32.gt_u (global.get $ptr) (local.get $mark)))
          (then
            (local.set $mark
              (call $scanFind3
                (global.get $ptr)
  ...
  ;; delimiter (`<<~`); bit 8 makes it a word closer that any non-letter may
  ;; follow (`=cut # end`; see $lineDelimiterEnds). An unterminated body
  ;; runs to $end and, in streaming, checkpoints the delimiter so the next
  ;; chunk keeps looking for it. Shared by heredocs and POD.
  (func $perlLineDelimited (param $delim i32) (param $n i32) (param $mode i32) (param $hl i32)
    (local $body i32)
    (local $lhs i32)
    (local $line i32)
  ...
  (func $hlPerl
    ...
    (block $done
      (loop $next
        ;; token tails, innermost first: $literal opens a quoted body,
        ...
        (if (local.get $hdLen)
          (then
            ...
            (local.set $c (call $perlByte (global.get $ptr)))
                  ...
                  (call $perlByte
                    (i32.add
                      ...
                      (select
                        (i32.const 8)
                        (i32.const 7)
                        (i32.eq (local.get $c3) (i32.const "D")))))))))
          (then
            (local.set $data (i32.const 1))
            (br $next)))

        ;; heredoc openers: `<<EOF`, `<<"EOF"`, `<<'EOF'`, `<<~EOF`, where a
        ;; term may start, or glued to a name after a blank the way a print
        ;; filehandle takes one (`print STDERR <<EOF`, `print $fh <<'SQL'`);
        ;; a digit before the blank keeps `1 <<BITS` a shift
        (if
            ...
            (if
              (i32.or
                ...
                (i32.and
                  ...
                  (i32.or
                    (i32.or
                      (call $lexIsDigit (local.get $kind))
                      (i32.eq (local.get $kind) (i32.const "{")))
                    (i32.and
                        ...
                        (br_if $pathDone
                          (i32.eqz
                            (i32.and
                              ...
                              (i32.and
                                (i32.eq
                                  (call $perlByte (i32.add (global.get $ptr) (i32.const 1)))
                                  (i32.const ":"))
                                (call $lexIsIdentStart
                                  (call $perlByte (i32.add (global.get $ptr) (i32.const 2))))))))
        ...
        (if (call $lexIsIdentStart (local.get $c))
          (then
            ...
            (local.set $kind
              (if (result i32) (local.get $member)
                (then (i32.const 0))
                (else (call $perlQuoteKind (local.get $lhs) (local.get $rhs)))))
            (if (local.get $kind)
              (then
                  ...
                  (i32.and
                    ...
                    (i32.eqz
                      (i32.or
                        ...
                        (i32.or
                          (i32.or
                            (i32.eq (local.get $sOpen) (i32.const ","))
                            (i32.eq (local.get $sOpen) (i32.const ";")))
                          (i32.or
                            (i32.or
                              (i32.eq (local.get $sOpen) (i32.const "="))
            ...
            (local.set $kind
              (if (result i32) (local.get $member)
                (then (i32.const -1))
                (else (call $perlWordHl (local.get $lhs) (local.get $rhs)))))
                    ...
                    (if
                      (i32.eqz
                        (i32.and
                          ...
                          (i32.eq
                            (call $perlByte (i32.add (local.get $rhs) (i32.const 1)))
                            (i32.const ":"))))
                                        ...
                                        (if
                                          (i32.or
                                            ...
                                            (i32.and
                                              ...
                                              (i32.eq
                                                (i32.load8_u
                                                  (i32.sub (local.get $lhs) (i32.const 1)))
                                                (i32.const "{"))))
                                          (then
        ...
        (br $opTok))
        ;; a quoted-body opener, colored by the body kind in $sFlags; the body
        ...
        (local.set $seg (global.get $ptr))
        (br $tok))
        ;; a value token: a `/` next is an operator, not a match
        (local.set $operand (i32.const 0))
        (br $tok))
