(module
  ...
  (keyword-table $powershellWords $mem.powershellWords $mem.protoWords
    (group ;; 1: control
      ...
      "return" "exit" "throw" "try" "catch" "finally" "trap" "begin" "process" "end" "dynamicparam"
      "default" "clean")
    ...
    (group ;; 4: declaration
      "param" "hidden" "static" "data" "define" "var" "inlinescript" "parallel" "sequence")
    ...
    (group ;; 6: dash operators, and the `in` of foreach
      ...
      "in" "notin" "replace" "creplace" "ireplace" "split" "join" "and" "or" "not" "xor" "band"
      "bor" "bxor" "bnot" "shl" "shr" "is" "isnot" "as" "ceq" "cne" "clike" "cmatch" "ieq" "ine"
      "ilike" "imatch")
    (group "true" "false") ;; 7: booleans, after `$`
    (group "null")         ;; 8: built-in constant, after `$`
    (group ;; 9: automatic variables, after `$`
      "this" "args" "input" "psitem" "pscmdlet" "psboundparameters" "matches" "error" "host" "pid"
      "pwd" "home" "psversiontable"))
    ...
    (block $done
      (loop $scan
        (if (local.get $expand)
          (then
            (local.set $p
              (call $scanFind3 (global.get $ptr) (local.get $q) (i32.const 96) (i32.const "$"))))
        ...
        (if (i32.eq (local.get $c) (i32.const 96))
          (then
            (call $emitTok (enum.get $Token.string) (local.get $seg) (local.get $p))
            (local.set $e
              (call $utf8SpanEnd (i32.add (local.get $p) (i32.const 2)) (global.get $end)))
          ...
          (then
            ...
            (block $commentDone
              (loop $comment
                ...
                (br $comment)))
            (global.set $ptr
              (select (i32.add (local.get $p) (i32.const 2)) (global.get $end) (local.get $closed)))
            (call $emitTok (enum.get $Token.comment) (local.get $lhs) (global.get $ptr))
            (if (i32.eqz (local.get $closed))
              (then
                (call $streamSetFixed32 (i32.const "#>") (i32.const 2) (enum.get $Token.comment))))
            (br $next)))
            ...
            (if (i32.eq (call $psByte (global.get $ptr)) (i32.const "("))
              (then
                ...
                (call $emitTok
                  ...
                  (local.get $lhs)
                  (global.get $ptr)))
              (else (call $emitTok (enum.get $Token.type) (local.get $lhs) (global.get $ptr))))
            (local.set $member (i32.const 0))
            (local.set $cmdPos (i32.const 0))
            (br $next)))
