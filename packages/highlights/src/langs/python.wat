(module
  ...
  (func $pyString (param $prefix i32) (param $quote i32) (param $raw i32) (param $format i32)
    (local $seg i32)
    (local $status i32)
    (local $triple i32)
    (local.set $seg (global.get $ptr))
  ...
  (keyword-table $pyWords $mem.pythonWords $mem.rWords
    ...
    (group $Token.keyword.control ;; 8: control keywords
      "assert" "async" "await" "break" "continue" "del" "elif" "else" "except" "finally" "for"
      "global" "if" "lambda" "nonlocal" "pass" "return" "try" "while" "with" "yield")
    ...
    (group $Token.function ;; 10: built-in functions
      "abs" "all" "any" "callable" "enumerate" "filter" "getattr" "input" "isinstance" "iter"
      "len" "map" "max" "min" "next" "open" "pow" "print" "repr" "round" "sorted" "sum" "super"
      "zip"))
                ...
                (if
                  (i32.and
                    ...
                    (i32.or
                      (i32.eq (local.get $c) (i32.const "]"))
                      (i32.eq (local.get $c) (i32.const "}"))))
                  (then (local.set $brackDepth (i32.sub (local.get $brackDepth) (i32.const 1)))))))
            (global.set $ptr (i32.add (global.get $ptr) (i32.const 1)))
          ...
          (then
            ...
            (local.set $typeNext
              (i32.and
                ...
                (i32.and
                  (local.get $annotName)
                  (i32.and (i32.eqz (local.get $brackDepth)) (i32.eqz (local.get $lambdaPend))))))
            (if (i32.eq (local.get $c) (i32.const ":"))
            ...
            (br $next)))
        (if (i32.or (call $pyIsOp (local.get $c)) (i32.eq (local.get $c) (i32.const ":")))
          (then
            (global.set $ptr (i32.add (global.get $ptr) (i32.const 1)))
            (if
              (i32.and
            ...
            (global.set $sigPattern
              (i32.and (global.get $sigPattern) (i32.eq (local.get $c) (i32.const "*"))))
            (br $next)))
        (global.set $ptr (i32.add (global.get $ptr) (i32.const 1)))
