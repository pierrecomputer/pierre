(module
  ...
  (keyword-table $nixWords $mem.nixWords $mem.objcWords
    ...
    (group $Token.function ;; 7: the built-in functions in scope
      "import" "throw" "abort" "derivation" "map" "toString" "baseNameOf" "dirOf" "removeAttrs"
      "fetchTarball" "fetchGit" "isNull"))
    ...
    (block $done
      (loop $scan
        (if (i32.eq (local.get $kind) (i32.const 1))
          ...
          (else
            (local.set $p (call $lexFindEither (global.get $ptr) (i32.const 39) (i32.const "$")))))
      ...
      (loop $next
        ...
        (if (local.get $strKind)
          (then
            ...
            (local.set $status
              (call $nixStringBody (local.get $strKind) (local.get $interp) (local.get $seg)))
            ...
            (if
              (i32.and
                ...
                (i32.and
                  ...
                  (i32.eq
                    (call $nixByte (i32.add (local.get $rhs) (i32.const 2)))
                    (i32.const "/"))))
              (then
                (block $uriDone
                  (loop $uri
              ...
              (loop $op
                ...
                (br_if $opDone
                  (i32.and
                    ...
                    (i32.eq
                      (call $nixByte (i32.add (global.get $ptr) (i32.const 1)))
                      (i32.const "*"))))
                (global.set $ptr (i32.add (global.get $ptr) (i32.const 1)))
                (br $op)))
            (call $emitTok (enum.get $Token.operator) (local.get $lhs) (global.get $ptr))
