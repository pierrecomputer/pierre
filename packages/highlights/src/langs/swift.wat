(module
  ...
  (func $swiftHashOpen (result i32)
    ...
    (i32.or
      (i32.shl (i32.add (local.get $hashes) (i32.const 1)) (i32.const 1))
      (local.get $triple)))
        ...
        (if (i32.eq (i32.load8_u (global.get $ptr)) (i32.const 92))
          (then
            ...
            (if (i32.eq (local.get $seen) (local.get $hashes))
              (then
                (global.set $ptr
                  (call $utf8SpanEnd (i32.add (local.get $q) (i32.const 1)) (global.get $end)))))
            (br $scan)))
        (if
          (i32.or
              ...
              (loop $stringScan
                ...
                (if (i32.eq (local.get $c) (i32.const 92))
                  (then
                    (if
                      (i32.and
                        (i32.eqz (local.get $interp))
                        (i32.eq
                          (call $swiftByte (i32.add (global.get $ptr) (i32.const 1)))
                          (i32.const "(")))
                      (then
                ...
                (br $stringScan)))
            (if
              (i32.and (i32.eqz (local.get $openedInterp)) (i32.ne (local.get $seg) (i32.const 0)))
              (then
                (call $emitTok (enum.get $Token.string) (local.get $seg) (global.get $ptr))
                (local.set $seg (i32.const 0))))
            (br $next)))
