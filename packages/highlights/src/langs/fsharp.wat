(module
  ...
  (keyword-table $fsharpWords $mem.fsharpWords $mem.gleamWords
    (group $Token.keyword.control ;; 1: control flow
      "if" "then" "elif" "else" "match" "with" "when" "for" "to" "downto" "while" "do" "done" "try"
      "finally" "raise" "failwith" "yield" "return" "begin" "end" "function" "fun" "lazy" "assert"
      "in")
    ...
    (group $Token.keyword.declaration ;; 6: other declarations and modifiers
      "as" "val" "static" "mutable" "inline" "interface" "inherit" "new" "class" "struct"
      "delegate" "of" "internal" "private" "public" "extern" "global" "upcast" "downcast" "base"
      "void" "fixed")
    ...
    (group $Token.type.builtin ;; 8: built-in types
      "int" "int8" "int16" "int32" "int64" "uint8" "uint16" "uint32" "uint64" "byte" "sbyte"
      "float" "float32" "double" "single" "decimal" "bigint" "bool" "char" "string" "unit" "obj"
      "exn" "list" "option" "seq" "array" "nativeint" "unativeint" "byref" "inref" "outref")
  ...
  (func $fsIsArgAt (param $p i32) (result i32)
    ...
    (if (i32.and (call $lexIsIdentStart (local.get $c)) (i32.ne (local.get $c) (i32.const "$")))
      (then
        (return
          (i32.lt_s (call $fsWordHl (local.get $p) (call $fsIdEnd (local.get $p))) (i32.const 0)))))
    (i32.const 1))
      ...
      (loop $next
        ...
        (if (local.get $strKind)
          (then
            ...
            (local.set $status
              (call $fsStringBody
                (i32.sub (local.get $strKind) (i32.const 1))
                (local.get $interp)
                (local.get $seg)))
            (local.set $seg (global.get $ptr))
            ...
            (call $lexLineComment
              ...
              (select
                ...
                (i32.and
                  ...
                  (i32.ne
                    (call $fsByte (i32.add (global.get $ptr) (i32.const 3)))
                    (i32.const "/")))))
            (br $next)))
        (if
          (i32.and
        ...
        (if
          (i32.and
            (i32.eq (local.get $c) (i32.const ">"))
            (i32.and (i32.eq (local.get $c2) (i32.const "]")) (local.get $attr)))
          (then
            (global.set $ptr (i32.add (global.get $ptr) (i32.const 2)))
            (call $emitTok (enum.get $Token.punctuation.special) (local.get $lhs) (global.get $ptr))
                      ...
                      (else
                        (if
                          ...
                          (then
                            ...
                            (local.set $lastModule
                              (i32.eq (call $fsByte (local.get $rhs)) (i32.const ".")))
              ...
              (loop $symbol
                ...
                (br_if $symbolDone
                  (i32.and
                    (i32.eq (i32.load8_u (global.get $ptr)) (i32.const "/"))
                    (i32.eq
                      (call $fsByte (i32.add (global.get $ptr) (i32.const 1)))
                      (i32.const "/"))))
                (br_if $symbolDone
                  (i32.and
