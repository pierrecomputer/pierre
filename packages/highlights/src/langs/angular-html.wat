  ...
  (func $angularExpression (param $state i32) (result i32)
    ...
    (block $done
      (loop $next
        ...
        (if (local.get $quote)
          (then
            (if (call $lexStringBody (local.get $quote) (i32.const 1) (enum.get $Token.string) (local.get $p))
          ...
          (then
            (if (i32.eq (local.get $c) (local.get $mode))
              (then
                ...
                (return (i32.const 3))))
            (if (i32.and (i32.eq (local.get $mode) (i32.const 5))
        ...
        (if (i32.eq (local.get $expr) (i32.const 5))
          (then
            (if (call $lexIsIdentStart (local.get $c))
              (then
                (call $lexScanIdent)
                (call $emitTok (call $angularKeyword (local.get $p) (global.get $ptr)) (local.get $p) (global.get $ptr))
                (br $next)))
            (if (i32.eq (local.get $c) (i32.const "("))
              (then
                (global.set $ptr (i32.add (local.get $p) (i32.const 1)))
                (call $emitTok (enum.get $Token.punctuation.bracket) (local.get $p) (global.get $ptr))
                (local.set $expr (i32.const 3))
                (br $next)))
            (if (i32.eq (local.get $c) (i32.const ";"))
              (then
                (global.set $ptr (i32.add (local.get $p) (i32.const 1)))
                (call $emitTok (enum.get $Token.punctuation.delimiter) (local.get $p) (global.get $ptr))))
            (return (i32.const 1))))
        (if (i32.eqz (local.get $depth))
          (then
            (if (i32.and (i32.eq (local.get $expr) (i32.const 1))
            ...
            (if (i32.or
                  (i32.and (i32.eq (local.get $expr) (i32.const 3)) (i32.eq (local.get $c) (i32.const ")")))
                  (i32.and (i32.eq (local.get $expr) (i32.const 4)) (i32.eq (local.get $c) (i32.const ";"))))
              (then
                (global.set $ptr (i32.add (local.get $p) (i32.const 1)))
                (call $emitTok (select (enum.get $Token.punctuation.bracket) (enum.get $Token.punctuation.delimiter)
                  (i32.eq (local.get $expr) (i32.const 3))) (local.get $p) (global.get $ptr))
                (return (i32.const 1))))))
        ...
        (if (call $lexIsIdentStart (local.get $c))
          (then
            ...
            (if (local.get $member)
              (then (local.set $hl (select (enum.get $Token.property) (enum.get $Token.function)
                (i32.eq (local.get $member) (i32.const 1))))))
            (if (i32.and (i32.ne (local.get $member) (i32.const 2))
                  (i32.eq (call $tsxByte (local.get $q)) (i32.const "(")))
              (then (local.set $hl (select (enum.get $Token.function.method) (enum.get $Token.function)
                (local.get $member)))))
        ...
        (if (i32.or (i32.eq (local.get $c) (i32.const "("))
          ...
          (then
            (local.set $depth (i32.add (local.get $depth) (i32.lt_u (local.get $depth) (i32.const 4095))))
            (local.set $hl (enum.get $Token.punctuation.bracket))))
        (if (i32.or (i32.eq (local.get $c) (i32.const ")"))
              (i32.or (i32.eq (local.get $c) (i32.const "]")) (i32.eq (local.get $c) (i32.const "}"))))
          (then
            (local.set $depth (i32.sub (local.get $depth) (i32.ne (local.get $depth) (i32.const 0))))
            (local.set $hl (enum.get $Token.punctuation.bracket))))
        (if (i32.or (i32.eq (local.get $c) (i32.const ";"))
              (i32.or (i32.eq (local.get $c) (i32.const ":")) (i32.eq (local.get $c) (i32.const ","))))
    ...
    (i32.or (local.get $mode)
      (i32.or (i32.shl (local.get $expr) (i32.const 7))
        (i32.or (i32.shl (local.get $quote) (i32.const 10))
          (i32.or (i32.shl (local.get $depth) (i32.const 17))
  ...
  (func $hlTemplateMarkup (param $state i32) (param $angular i32) (result i32)
    ...
    (block $done
      (loop $next
        ...
        (local.set $p (global.get $ptr))
        (local.set $c (i32.load8_u (local.get $p)))
        (if (i32.and (local.get $angular) (i32.ne (i32.and (local.get $state) (i32.const 896)) (i32.const 0)))
          (then
        ...
        (if (i32.or (i32.eq (local.get $mode) (i32.const 34))
          ...
          (then
            (if (local.get $angular)
              (then
                (global.set $ptr (call $lexFindEither (global.get $ptr) (local.get $mode) (i32.const "{")))
                (if (i32.eq (call $tsxByte (global.get $ptr)) (i32.const "{"))
                  (then
              ...
              (then
                ...
                (if (i32.eq (local.get $c) (i32.const "@"))
                  (then
                    (global.set $ptr (i32.add (local.get $p) (i32.const 1)))
                    (call $lexScanIdent)
                    (local.set $hl (call $angularKeyword (i32.add (local.get $p) (i32.const 1)) (global.get $ptr)))
                    (if (i32.or (i32.eq (local.get $hl) (enum.get $Token.keyword.control))
                                (i32.eq (local.get $hl) (enum.get $Token.keyword.declaration)))
                      (then
                        (call $emitTok (local.get $hl) (local.get $p) (global.get $ptr))
                        (local.set $state (select (i32.const 513) (i32.const 641)
                          (i32.eq (local.get $hl) (enum.get $Token.keyword.declaration))))
                        (br $next)))
              ...
              (then
                ...
                (block $textEnd
                  (loop $text
                    ...
                    (br_if $textEnd (i32.or
                      (i32.or (i32.eq (local.get $c) (i32.const "<")) (i32.eq (local.get $c) (i32.const "&")))
                      (i32.or (i32.eq (local.get $c) (i32.const "@"))
                        (i32.or (i32.eq (local.get $c) (i32.const "{")) (i32.eq (local.get $c) (i32.const "}"))))))
                    (global.set $ptr (i32.add (global.get $ptr) (i32.const 1)))
                    (br $text))))
              (else (global.set $ptr (call $lexFindEither
        ...
        (if (i32.eq (local.get $c) (i32.const ">"))
          ...
          (else
            ...
            (if (i32.or (i32.eq (local.get $c) (i32.const 34))
              ...
              (then
                (local.set $state (i32.or (local.get $c)
                  (select (i32.const 256) (i32.const 0) (i32.and (local.get $state) (i32.const 64)))))
                (call $emitTok (enum.get $Token.string) (local.get $p) (global.get $ptr))
                (br $next)))
            ...
            (if (i32.eq (local.get $c) (i32.const "="))
              (then
                (local.set $state (i32.or (i32.const 4) (i32.and (local.get $state) (i32.const 64))))
              ...
              (else
                (if (i32.ne (local.get $c) (i32.const "/"))
                  (then
                    ...
                    (local.set $state (i32.or (i32.const 3)
                      (select (i32.const 64) (i32.const 0)
                        (i32.and (local.get $angular)
                          (i32.and (i32.ne (local.get $mode) (i32.const 2))
                            (i32.or (i32.eq (local.get $c) (i32.const "["))
                              (i32.or (i32.eq (local.get $c) (i32.const "("))
                                      (i32.eq (local.get $c) (i32.const "*")))))))))))))))
        (call $emitTok (local.get $hl) (local.get $p) (global.get $ptr))
