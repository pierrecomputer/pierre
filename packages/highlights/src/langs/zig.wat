(module
  ...
  (func $zigScanDigits (param $p i32) (param $base i32) (result i32)
    ...
    (block $done
      (loop $digit
        (br_if $done (i32.ge_u (local.get $p) (global.get $end)))
        (if (call $zigIsDigitForBase (i32.load8_u (local.get $p)) (local.get $base))
          (then
        ...
        (br_if $done
          (i32.or
            ...
            (i32.eqz
              (call $zigIsDigitForBase (i32.load8_u offset=1 (local.get $p)) (local.get $base)))))
        (local.set $p (i32.add (local.get $p) (i32.const 1)))
        (br $digit)))
    (local.get $p))
            ...
            (if
              ...
              (then
                ...
                (if (i32.eq (local.get $c) (i32.const "x"))
                  (then (local.set $e (call $scanHexRun (local.get $e) (i32.const 2)))))
                ...
                (if
                  ...
                  (then
                    (local.set $e
                      (call $scanHexRun (i32.add (local.get $e) (i32.const 1)) (i32.const 6)))
  ...
  (func $hlZig
    ...
    (block $done
      (loop $next
        ...
        (if (i32.and (i32.eq (local.get $c) (i32.const "@")) (i32.eq (local.get $p) (i32.const 34)))
          (then
        ...
        (if (i32.and (i32.eq (local.get $c) (i32.const "@")) (call $zigIsIdentStart (local.get $p)))
          (then
            ...
            (if
              (i32.or
                (i32.and
                  (i32.eq (i32.sub (global.get $ptr) (local.get $lhs)) (i32.const 7))
                  (i64.eq
        ...
        (if (call $lexIsDigit (local.get $c))
          (then
            ...
            (if (i32.eq (local.get $c) (i32.const "0"))
              (then
                (local.set $c (call $zigByte (i32.add (local.get $lhs) (i32.const 1))))
            ...
            (if
              ...
              (then
                (global.set $ptr
                  (call $zigScanDigits (i32.add (local.get $lhs) (i32.const 2)) (local.get $base))))
              (else
                (local.set $base (i32.const 10))
                (global.set $ptr (call $zigScanDigits (local.get $lhs) (i32.const 10)))))
            (if
              (i32.and
                (i32.or
              ...
              (then
                (global.set $ptr
                  (call $zigScanDigits
                    ...
                    (local.get $base)))))
            (local.set $c (i32.or (call $zigByte (global.get $ptr)) (i32.const 32)))
            (if
              (i32.or
                (i32.and
                  (i32.eq (local.get $base) (i32.const 10))
              ...
              (then
                ...
                (if (call $lexIsDigit (call $zigByte (local.get $p)))
                  (then (global.set $ptr (call $zigScanDigits (local.get $p) (i32.const 10)))))))
        ...
        (if (call $zigIsIdentStart (local.get $c))
          (then
            (global.set $ptr (i32.add (global.get $ptr) (i32.const 1)))
            (global.set $ptr (call $zigIdentEnd (global.get $ptr)))
            (local.set $hl (call $zigWordHl (local.get $lhs) (global.get $ptr)))
              ...
              (then
                ...
                (if
                  ...
                  (then
                    ...
                    (local.set $word (keyword-table.get $zigWords (local.get $q) (local.get $p)))
        ...
        (if (byteset.get "()[]{}" (local.get $c))
          (then
            (if (i32.eq (local.get $c) (i32.const "("))
              (then
                (local.set $parenDepth (i32.add (local.get $parenDepth) (i32.const 1)))
                (if (local.get $wantPayloadParen)
                  (then
                    (local.set $payloadParens
                      (i64.or
                        (local.get $payloadParens)
                        (i64.shl (i64.const 1) (i64.extend_i32_u (local.get $parenDepth)))))))
                ...
                (local.set $payloadParens
                  (i64.and
                    ...
                    (i64.xor
                      (i64.shl (i64.const 1) (i64.extend_i32_u (local.get $parenDepth)))
                      (i64.const -1))))
                (local.set $parenDepth (i32.sub (local.get $parenDepth) (i32.const 1)))))
                ...
                (if (i32.eq (local.get $c) (i32.const ")"))
                  (then
                    ...
                    (if (i32.gt_u (global.get $sigParens) (i32.const 0))
                      (then
                        (global.set $sigParens (i32.sub (global.get $sigParens) (i32.const 1))))))
                  (else
                    (if (call $sigActive)
                      (then
                        (if
                  ...
                  (then
                    (local.set $base
                      (select
                        ...
                        (i32.const 2)
                        (i32.eq (local.get $q) (i32.const "="))))))
                (global.set $ptr (i32.add (global.get $ptr) (local.get $base)))
                (br_if $op
                  (i32.and
