...
void t.test(
  'LiveTokenizer: bounded flush leaves the tail pending and resumes it',
  async () => {
    const code = '/*\n' + 'inside\n'.repeat(30) + '*/';
    ...
    try {
      ...
      live.flush(4);
      assert.deepEqual([...delivered.keys()], [2, 3]);
      assert.deepEqual(live.getLineTokens(3).bracketIgnoredRanges, [[0, 6]]);
      assert.ok(live.pendingTokenization);
      live.flush(0);
      assert.deepEqual([...delivered.keys()], [2, 3]);
      assert.throws(() => live.flush(-1));
      assert.throws(() => live.flush(1.5));
      const deadline = Date.now() + 2000;
      while (live.pendingTokenization && Date.now() < deadline)
        await new Promise((resolve) => setTimeout(resolve, 0));
      assert.equal(live.pendingTokenization, false);
      assertMatchesFresh(live, code, 'ts', 'bounded flush');
    } finally {
      live.dispose();
    }
  }
...
void t.test(
  'LiveTokenizer: range limits survive jumps between disjoint edits',
  () => {
    for (const flush of [false, true]) {
      ...
      try {
        const update = live.applyEdits(
          ...
          { renderRange: [0, flush ? 0 : 10] }
        );
        if (flush) live.flush(10);
        assert.deepEqual([...(flush ? delivered : update.lines).keys()], [0]);
        assert.ok(live.pendingTokenization);
        live.flush();
        assert.ok(delivered.has(20));
        assertMatchesFresh(
          live,
          live.getText(),
          'ts',
          'disjoint bounded update'
        );
      } finally {
        live.dispose();
      }
