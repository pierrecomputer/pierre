...
if (import.meta.main) {
  ...
  const sourceUrl = new URL('../src/chamele.wat', moduleUrl);
  ...
  console.log(
    `✨ Done in ${Math.ceil(performance.now() - start)}ms (wasm: ${pkg.meta['chamele.wasm']} bytes, gzipped: ${pkg.meta['chamele.wasm.gz']} bytes, -O3)`
  );
}
