...
export class FileDiff<LAnnotation = undefined> {
  ...
  async dumbRender(
    props: FileDiffRenderProps<LAnnotation>
    ...
    const [highlighter, hunksResult] = await Promise.all([
      getSharedHighlighter({ themes: this.getThemes(), langs: [] }),
      this.hunksRenderer.render(this.fileDiff),
    ]);

    // This is just so i dont get warnings for now for not using this
    if (highlighter == null) {
    ...
    if (hunksResult != null) {
      const { additionsHTML, deletionsHTML, css } = hunksResult;
      const preClass = `pre-wrapper`;
      const deletionsClass = `deletions-code`;
      const additionsClass = `additions-code`;
      const wrapperStyles = `.${preClass} {
  color: rgb(251, 251, 251);
  background-color: rgb(7, 7, 7);
  --pjs-fg: #fbfbfb;
  --pjs-bg: #070707;
  --pjs-addition-color: #00cab1;
  --pjs-deletion-color: #ff2e3f;
  --pjs-modified-color: #08c0ef;
}
.${deletionsClass} {
  --pjs-column-content-width: 389px;
  --pjs-column-width: 432px;
  --pjs-column-number-width: 43px;
}
.${additionsClass} {
  --pjs-column-content-width: 389px;
  --pjs-column-width: 432px;
...
${
  ...
  data-code
  data-additions
  class="${additionsClass}"
>${additionsHTML}</code>`
    : ''
}
</pre>`;

      return { html, css: wrapperStyles + (css ?? '') };
    }
    return { html: 'no diff detected', css: '' };
  }
