...
async function openFixture(page: Page) {
  await page.goto('/test/e2e/fixtures/path-store-style-isolation.html');
  await page.waitForFunction(
    () => window.__pathStoreStyleIsolationReady === true
  );
}

test.describe('path-store file-tree shadow style isolation', () => {
...
declare global {
  interface Window {
    __pathStoreStyleIsolationReady?: boolean;
  }
}
