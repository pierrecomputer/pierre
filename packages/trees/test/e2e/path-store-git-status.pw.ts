...
declare global {
  interface Window {
    __pathStoreGitStatusFixtureReady?: boolean;
  }
}

async function openFixture(page: Page) {
  await page.goto('/test/e2e/fixtures/path-store-git-status.html');
  await page.waitForFunction(
    () => window.__pathStoreGitStatusFixtureReady === true
  );
}
...
async function readGitStatusSnapshot(page: Page) {
  return page.evaluate(() => {
    ...
    return {
      ...
      state:
        document.querySelector('[data-path-store-git-status-state]')
          ?.textContent ?? null,
...
test.describe('path-store git-status proof', () => {
  test('renders git-status attrs and updates them in place across set A, set B, and disabled states', async ({
    ...
    await page.locator('[data-path-store-git-status-action="set-b"]').click();
    ...
    await page
      .locator('[data-path-store-git-status-action="toggle-enabled"]')
      .click();
