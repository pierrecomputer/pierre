...
export interface PathStoreResolvedIcon {
  height?: number;
  name: string;
  remappedFrom?: string;
  token?: string;
...
// Mirrors the legacy file-icon priority order so the path-store lane can gain
// icon tiers without creating a second icon rule system.
export function createPathStoreIconResolver(icons?: FileTreeIcons): {
  resolveIcon: (
    name: SVGSpriteNames,
    filePath?: string
  ) => PathStoreResolvedIcon;
