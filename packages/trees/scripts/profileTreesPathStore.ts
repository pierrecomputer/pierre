...
import {
  ...
  type PathStoreProfileWorkloadName,
} from './lib/pathStoreProfileShared';

interface ProfileConfig {
  browserUrl: string;
  url: string;
  workloads: PathStoreProfileWorkloadName[];
  timeoutMs: number;
  runs: number;
  warmupRuns: number;
...
type PageRenderSummary = PathStoreProfilePageSummary;
...
interface ProfileBenchmarkOutput {
  benchmark: 'treesPathStoreProfile';
...
declare global {
  interface Window {
    ...
    __treesPathStoreProfile?: PageRenderSummary;
  }
}
...
const DEFAULT_URL =
  'http://127.0.0.1:9221/test/e2e/fixtures/path-store-profile.html';
const DEFAULT_WORKLOAD_NAME = DEFAULT_PATH_STORE_PROFILE_WORKLOAD_NAME;
const KNOWN_WORKLOAD_NAMES = new Set<PathStoreProfileWorkloadName>(
  PATH_STORE_PROFILE_WORKLOAD_NAMES
);
...
const DEFAULT_TRACE_OUTPUT_EXAMPLE_PATH = resolve(
  ...
  'trees-path-store-profile-trace-<run-id>.json'
);
const START_MARK_NAME = 'trees-path-store-profile-start';
const END_MARK_NAME = 'trees-path-store-profile-end';
const START_TRACE_LABEL = 'trees-path-store-profile-trace-start';
const END_TRACE_LABEL = 'trees-path-store-profile-trace-end';
...
function printHelpAndExit(): never {
  console.log('Usage: bun ws trees profile:pathstore -- [options]');
  console.log('');
  console.log(
  ...
  console.log(
    '  --compare <path>       Compare against a prior --json path-store profile run'
  );
  console.log(
...
function parseWorkloadName(value: string): PathStoreProfileWorkloadName {
...
function createDefaultTraceOutputPath(): string {
  return resolve(
    DEFAULT_TRACE_OUTPUT_DIR,
    `trees-path-store-profile-trace-${createTraceRunId()}.json`
  );
}
...
function isPathStoreProfileFixtureUrl(url: string): boolean {
  try {
...
function createProfileUrl(
  ...
  const parsedUrl = new URL(url);
  if (isPathStoreProfileFixtureUrl(url)) {
...
async function navigateToFixture(
  ...
  const ready = await evaluateJson<boolean>(
    ...
    `(async () => {
      ...
      while (performance.now() - started < ${timeoutMs}) {
        if (window.__treesPathStoreFixtureReady === true) {
          return true;
        }
  ...
  if (!ready) {
    throw new Error(
      'Timed out waiting for the path-store profile fixture to load.'
    );
  }
    ...
    `(async () => {
      ...
      while (performance.now() - started < ${timeoutMs}) {
        if (window.__treesPathStoreProfile != null) {
          return { done: true, profile: window.__treesPathStoreProfile };
        }
        await new Promise((resolve) => setTimeout(resolve, 50));
      }
      return {
        done: false,
        profile: window.__treesPathStoreProfile ?? null,
      };
    })()`
  );

  if (!summary.done || summary.profile == null) {
    throw new Error('Timed out waiting for the path-store render summary.');
  }
...
async function profilePathStoreRender(
  config: ProfileConfig,
  workloadName: string,
  runNumber: number,
  traceOutputPath: string | null
...
function normalizeProfileBenchmarkOutput(
  ...
  if (
    ...
    rawValue.benchmark !== 'treesPathStoreProfile'
  ) {
...
async function runWorkloadProfile(
  ...
  for (let runNumber = 1; runNumber <= config.runs; runNumber += 1) {
    ...
    const result = await profilePathStoreRender(
      config,
      workloadName,
      runNumber,
      traceOutputPath
