...
try {
	...
	console.error(error instanceof Error ? error.message : 'Failed to parse command line arguments.');
	printUsage();
	process.exit(1);
}
...
async function waitFor(check, options = {}) {
	const { timeout = 120_000, interval = 2_000, description, isFatalError, onRetry } = options;
...
async function main() {
	...
	await waitFor(
		async () => {
			...
			return branches.branches.find(
				(branch) => branch.name === defaultBranch && branch.headSha === latestCommitSha,
			);
		},
		{ timeout, description: `default branch ${defaultBranch} to include initial commit` },
	);
	console.log(`✓ Default branch updated (${defaultBranch})`);

	await waitFor(
		async () => {
			const commits = await repo.listCommits({ branch: defaultBranch, limit: 5 });
	...
	await waitFor(
		...
		{
			timeout,
			description: 'grep API to return empty results for case-sensitive mismatch',
			isFatalError: (error) =>
	...
	const grepCaseInsensitive = await waitFor(
		async () => {
			...
			const hasMatch = result.matches.some((match) =>
				match.lines.some((line) => line.type === 'match' && line.text.includes('ONLYUPPERCASE')),
			);
		...
		},
		{
			...
			onRetry: ({ attempt, error }) => {
				...
				if (error) {
					console.log(`… waiting for grep results (attempt ${attempt}): ${error.message ?? error}`);
				} else {
	...
	const grepFiltered = await waitFor(
		async () => {
			...
			const hasToken = result.matches.some((match) =>
				match.lines.some((line) => line.type === 'match' && line.text.includes(grepToken)),
			);
		...
		},
		{
			...
			onRetry: ({ attempt, error }) => {
				...
				if (error) {
					...
					console.log(`… waiting for grep file-filtered match (attempt ${attempt})`);
				}
			},
		},
	);
	console.log(`✓ Grep API respects file filters (${grepFiltered.matches.length} file(s))`);
	...
	await waitFor(
		async () => {
			const commits = await repo.listCommits({ branch: defaultBranch, limit: 5 });
			const match = commits.commits.find((commit) => commit.sha === latestCommitSha);
			if (match) {
	...
	await waitFor(
		...
		{ timeout, description: `default branch ${defaultBranch} to advance to commit-pack add` },
	);
	...
	await waitFor(
		...
		{ timeout, description: 'api-generated.txt to appear in listFiles response' },
	);
	...
	if (updateInfo.message !== updateMessage) {
		throw new Error(`Unexpected commit message for update: ${updateInfo.message}`);
	}
	...
	await waitFor(
		...
		{ timeout, description: `default branch ${defaultBranch} to advance to commit-pack update` },
	);
	...
	const diff = await waitFor(
		async () => {
			...
			return response.files.some((file) => file.path === 'api-generated.txt') ? response : null;
		},
	...
	await waitFor(
		async () => {
			const response = await repo.getFileStream({ path: 'api-generated.txt', ref: defaultBranch });
	...
	if (diffCommitInfo.message !== diffMessage) {
		throw new Error(`Unexpected diff commit message: ${diffCommitInfo.message}`);
	}
	...
	await waitFor(
		async () => {
			const response = await repo.getFileStream({ path: diffFileName, ref: defaultBranch });
			const body = await response.text();
	...
	await waitFor(
		async () => {
			...
			return branches.branches.find(
				(branch) => branch.name === featureBranch && branch.headSha === featureCommit.commitSha,
			);
		},
		{ timeout, description: `feature branch ${featureBranch} to appear in branch list` },
	);
	...
	if (restoreInfo.message !== restoreMessage) {
		throw new Error(`Unexpected restore commit message: ${restoreInfo.message}`);
	}
	...
	await waitFor(
		...
		{ timeout, description: `default branch ${defaultBranch} to advance to restore commit` },
	);
	...
	await waitFor(
		async () => {
			const files = await repo.listFiles({ ref: defaultBranch });
			return files.paths.some((path) => path === 'api-generated.txt' || path === diffFileName)
				? null
				: files;
		},
		{
			timeout,
			description: 'api-generated.txt and diff-endpoint.txt removed after restore commit',
		},
	);
	console.log(`✓ api-generated.txt and ${diffFileName} removed by restore commit`);
...
function patchGitStorage(GitStorage) {
	...
	GitStorage.prototype.generateJWT = async function patchedGenerateJWT(repoId, options) {
