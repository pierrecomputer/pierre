...
function ensureMockFetch(): MockFetch {
	...
	if (existing && typeof existing === 'function' && 'mock' in (existing as any)) {
...
describe('createCommit builder', () => {
	...
	it('streams metadata and blob chunks in NDJSON order', async () => {
		...
		mockFetch.mockImplementationOnce(async (_url, init) => {
			...
			const decoded = Buffer.from(chunkFrame.blob_chunk.data, 'base64').toString('utf8');
	...
	it('allows base_branch without expectedHeadSha', async () => {
		...
		mockFetch.mockImplementationOnce(() =>
			Promise.resolve({
				ok: true,
				status: 200,
				json: async () => ({ repo_id: 'repo-base-no-head', url: 'https://repo.git' }),
			}),
		);
	...
	it('includes ephemeral flags when requested', async () => {
		...
		mockFetch.mockImplementationOnce(() =>
			Promise.resolve({
				ok: true,
				status: 200,
				json: async () => ({ repo_id: 'repo-ephemeral', url: 'https://repo.git' }),
			}),
		);
	...
	it('accepts Blob and ReadableStream sources', async () => {
		...
		mockFetch.mockImplementationOnce(async (_url, init) => {
			...
			expect(metadata.files).toEqual([
				expect.objectContaining({ path: 'assets/blob.bin', content_id: 'blob-source' }),
				expect.objectContaining({ path: 'assets/stream.bin', content_id: 'stream-source' }),
			]);
	...
	it('splits large payloads into <=4MiB chunks', async () => {
		...
		mockFetch.mockImplementationOnce(async (_url, init) => {
			...
			expect(Buffer.from(firstChunk.data, 'base64')).toHaveLength(MAX_CHUNK_BYTES);
	...
	it('throws when author is missing', async () => {
		...
		mockFetch.mockImplementationOnce(() =>
			Promise.resolve({
				ok: true,
				status: 200,
				json: async () => ({ repo_id: 'repo-missing-author', url: 'https://repo.git' }),
			}),
		);

		const repo = await store.createRepo({ id: 'repo-missing-author' });
		expect(() =>
			repo.createCommit({ targetBranch: 'main', commitMessage: 'Missing author' }),
	...
	it('accepts legacy targetRef for backwards compatibility', async () => {
		...
		mockFetch.mockImplementationOnce(() =>
			Promise.resolve({
				ok: true,
				status: 200,
				json: async () => ({ repo_id: 'repo-legacy-target-ref', url: 'https://repo.git' }),
			}),
		);
	...
	it('honors deprecated ttl option when sending commits', async () => {
		...
		mockFetch.mockImplementationOnce(() =>
			Promise.resolve({
				ok: true,
				status: 200,
				json: async () => ({ repo_id: 'repo-legacy-ttl', url: 'https://repo.git' }),
			}),
		);
	...
	it('includes Code-Storage-Agent header in commit requests', async () => {
		...
		mockFetch.mockImplementationOnce(() =>
			Promise.resolve({
				ok: true,
				status: 200,
				json: async () => ({ repo_id: 'repo-user-agent', url: 'https://repo.git' }),
			}),
		);
