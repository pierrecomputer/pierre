...
describe('Webhook Validation', () => {
	...
	describe('validateWebhookSignature', () => {
		...
		it('should validate with Buffer payload', async () => {
			...
			const result = await validateWebhookSignature(payloadBuffer, header, secret);
		...
		it('should reject old timestamp (replay protection)', async () => {
			...
			const { header } = await generateSignature(payloadStr, secret, oldTimestamp);
			const result = await validateWebhookSignature(payloadStr, header, secret);
		...
		it('should reject future timestamp', async () => {
			...
			const { header } = await generateSignature(payloadStr, secret, futureTimestamp);
			const result = await validateWebhookSignature(payloadStr, header, secret);
		...
		it('should use custom max age', async () => {
			...
			const result1 = await validateWebhookSignature(payloadStr, header, secret, {
				maxAgeSeconds: 30,
			});
			expect(result1.valid).toBe(false);

			// Should succeed with 120 second max age
			const result2 = await validateWebhookSignature(payloadStr, header, secret, {
				maxAgeSeconds: 120,
			});
		...
		it('should reject malformed signature header', async () => {
			const result = await validateWebhookSignature(payloadStr, 'invalid_header', secret);
			expect(result).toEqual({
				valid: false,
		...
		it('should handle different payload modifications', async () => {
			...
			const result1 = await validateWebhookSignature(modifiedPayload, header, secret);
			expect(result1.valid).toBe(false);
			...
			const result3 = await validateWebhookSignature(payloadBuffer, header, secret);
	...
	describe('validateWebhook', () => {
		...
		it('should pass through validation options', async () => {
			...
			const result2 = await validateWebhook(payloadStr, headers, secret, { maxAgeSeconds: 0 });
			expect(result2.valid).toBe(true);
	...
	describe('Security considerations', () => {
		it('should use constant-time comparison', async () => {
			...
			const result = await validateWebhookSignature(payloadStr, shortSigHeader, secret);
		...
		it('should handle empty or undefined inputs safely', async () => {
			...
			// Empty payload
			expect((await validateWebhookSignature('', header, secret)).valid).toBe(false);

			// Empty secret
			expect((await validateWebhookSignature(payloadStr, header, '')).valid).toBe(false);

			// Empty header
			expect((await validateWebhookSignature(payloadStr, '', secret)).valid).toBe(false);
		});
