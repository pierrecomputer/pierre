...
worker pool options to use Highlights or Shiki's JavaScript engine. Each backend
loads on demand through `@pierre/diffs/highlighter/highlights`,
`@pierre/diffs/highlighter/shiki-wasm`, or `@pierre/diffs/highlighter/shiki-js`;
Shiki themes and languages also load on demand. Both Shiki engines support
incremental editor and stream tokenization. Custom theme loaders should return
the selected backend's theme format.
...
Use `resolveTheme(name, preferredHighlighter?)` to load a bundled or registered
theme and cache it in the shared highlighter. It defaults to `'shiki-wasm'` and
returns the selected backend's theme format.

```ts
import { resolveTheme } from '@pierre/diffs';

const shikiTheme = await resolveTheme('pierre-dark');
const highlightsTheme = await resolveTheme('pierre-dark', 'highlights');
```
...
Use `preloadHighlighter({ themes, langs, preferredHighlighter })` to load
languages before rendering. For worker pools, set `highlighterOptions.langs` to
preload languages on the main thread and in every worker before the pool is
ready. Both `'shiki-wasm'` and `'shiki-js'` support bundled and registered
custom languages; `'highlights'` ignores `langs` because its languages are
bundled.
