...
interface HighlighterOptions {
  ...
  /** Languages to preload for Shiki. Ignored by Highlights. */
  langs?: string[];
...
export const defaultHighlighter: HighlighterName = 'shiki-wasm';
...
/**
 * Get or create the shared instance for the selected backend (shiki-wasm by
 * default). Concurrent calls share initialization. Resolves after the requested
 * themes and any languages requiring loading are ready.
 */
...
/** Resolve and cache a theme in the selected backend's format. Defaults to Shiki with WebAssembly. */
export async function resolveTheme(
  themeName: DiffsThemeNames,
  preferredHighlighter: HighlighterName = defaultHighlighter
): Promise<DiffsTheme> {
 ...
 * themes and languages are ready. Defaults to Shiki with WebAssembly and returns
...
export function registerCustomTheme(
  ...
  const instances = [...cache.values()].filter(
 ...
 * shared shiki-wasm cache entry; does not check theme or language readiness.
 */
...
/**
 * Check whether a cached value is absent. Defaults to the shared shiki-wasm
 * cache entry.
 */
 ...
 * languages before rendering. Highlights ignores `langs` because its languages
 * are bundled. Defaults to Shiki with WebAssembly.
 */
