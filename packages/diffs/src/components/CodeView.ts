...
interface CodeViewElementPoolStats {
  acquiredFromPool?: number;
  created?: number;
...
export class CodeView<LAnnotation = undefined> {
  ...
  private elementPoolDisabled: boolean;
  ...
  constructor(
    ...
    this.elementPoolDisabled = false;
  ...
  private acquireElement(): HTMLElement {
    if (this.elementPoolDisabled) {
      incrementElementPoolStat('created');
    ...
    if (element != null) {
      incrementElementPoolStat('acquiredFromPool');
      return element;
    }
  ...
  private releaseRenderedItem(item: CodeViewContextItem<LAnnotation>): void {
    ...
    element.remove();
    if (this.elementPoolDisabled) {
      incrementElementPoolStat('disabledReleases');
      return;
    }
  ...
  private queueElementForPooling(element: HTMLElement): void {
    if (this.getElementPoolSize() >= CODE_VIEW_ELEMENT_POOL_LIMIT) {
      incrementElementPoolStat('dropped');
      return;
    }

    if (this.isElementReadyForPool(element)) {
      incrementElementPoolStat('queuedReady');
      this.elementPool.push(element);
    } else {
      incrementElementPoolStat('queuedPending');
  ...
  private promotePendingPooledElements(): void {
    ...
    for (const element of pending) {
      if (
        ...
        incrementElementPoolStat('promoted');
        this.elementPool.push(element);
...
function incrementElementPoolStat(key: keyof CodeViewElementPoolStats): void {
  const global = globalThis as typeof globalThis & {
    __DIFFS_ELEMENT_POOL_STATS?: CodeViewElementPoolStats;
  };
