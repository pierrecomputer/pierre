...
import { getEditIconSvg, type SVGSpriteNames } from './sprite';
...
export class SearchPanelWidget {
  ...
  constructor(options: SearchPanelOptions) {
    ...
    // handling for free; `getEditIconSvg` marks the glyph aria-hidden, so the
    ...
    const iconButton = (opts: {
      ...
      h('button', {
        ...
        innerHTML: getEditIconSvg(opts.icon, opts.size ?? 16),
    ...
    // Cells are positioned by CSS grid-template-areas (see edit.css), so DOM
