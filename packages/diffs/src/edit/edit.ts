...
import {
  type EditCommand,
  resolveEditCommandFromKeyboardEvent,
  resolveFindAgainShortcut,
} from './command';
import editCSS from './edit.css?inline';
...
import { EditTokenizer, renderLineTokens } from './tokenzier';
import {
  addEventListener,
  clampDomOffset,
  extend,
...
export interface EditOptions<LAnnotation> {
...
export class Edit<LAnnotation> implements DiffsEdit<LAnnotation> {
  #options: EditOptions<LAnnotation>;
  #metrics = new Metrics();
  #tokenizer?: EditTokenizer;
  #popoverManager?: PopoverManager;

  // event disposes
  #editEventDisposes?: (() => void)[];
  ...
  #editStyleElement?: HTMLStyleElement;
  ...
  // edits, without regressing a same-tick setSelections-then-applyEdits flow.
  #contentHasFocus = false;
  #isComposing = false;
  #isGutterMouseDown = false;
  #isContentMouseDown = false;
  #shiftKeyPressed = false;
  #selectionStart: EditSelection | undefined;
  ...
  #reservedSelections?: EditSelection[];
  #initSelections?: EditSelection[];
  ...
  constructor(options: EditOptions<LAnnotation> = {}) {
    this.#options = options;
  }

  setOptions(options: EditOptions<LAnnotation>): void {
    this.#options = {
      ...this.#options,
      ...options,
    };
  ...
  edit(fileInstance: DiffsEditableComponent<LAnnotation>): () => void {
    ...
    this.#initialize();
    this.#detach = fileInstance.attachEdit(this);
    return () => this.cleanUp();
  }
  ...
  applyEdits(edits: TextEdit[], updateHistory = false): void {
    ...
    let nextSelections: EditSelection[] | undefined;
    if (
      selectionsBefore !== undefined &&
  ...
  setState({ selections, view }: EditState): void {
  ...
  setSelections(
    ...
    const resolvedSelections = selections.map<EditSelection>((selection) => {
  ...
  cleanUp(recycle = false): void {
    ...
    this.#editEventDisposes?.forEach((dispose) => dispose());
    this.#editEventDisposes = undefined;
    ...
    this.#editStyleElement?.remove();
    this.#editStyleElement = undefined;
  ...
  /** @internal */
  __syncRenderView: DiffsEdit<LAnnotation>['__syncRenderView'] = (
    ...
    if (shadowRoot == null) {
      console.error('[edit] Could not find the shadow root.');
      return;
    }
    ...
    // inject edit&theme style to the file container
    if (this.#fileContainer !== fileContainer) {
      ...
      if (this.#editStyleElement !== undefined) {
        shadowRoot.appendChild(this.#editStyleElement);
      }
    ...
    if (shouldRebuildDocument) {
      ...
      if (this.#textDocument !== undefined && this.#options.__debug === true) {
        console.log('[diffs/edit] text document rebuilt from', fileOrDiff.name);
      }
    }
    ...
    if (this.#tokenizer == null && textDocument != null) {
      this.#tokenizer = new EditTokenizer({
        highlighter,
        textDocument,
    ...
    if (this.#contentElement !== contentEl) {
      ...
      if (
        ...
        console.log('[diffs/edit] full re-render triggered !!!');
      }
    }
    ...
    this.#globalEventDisposes = [
      addEventListener(
        ...
        () => {
          ...
          // overwrite multi-cursor edit state outside an active pointer gesture.
          if (
            this.#selections !== undefined &&
          ...
          if (
            ...
            !this.#rangeBelongsToEdit(composedRange)
          ) {
            return;
          }
    ...
    this.#editEventDisposes = [
      ...
      addEventListener(
        ...
        (e) => {
          ...
          // edit's own selection, so only one region is highlighted at a
      ...
      addEventListener(contentEl, 'keydown', (e) => {
        ...
        const command = resolveEditCommandFromKeyboardEvent(e);
  ...
  #computeContentOffset(contentEl: HTMLElement) {
    if (this.#isDiff && this.#diffSyle === 'split' && this.#isWrap) {
      ...
      if (this.#options.__debug === true) {
        console.log('[diffs/edit] content offset:', this.#contentOffset);
      }
    }
  }
  ...
  // same edit keeps a stale offset. Read it through this getter, which returns
  ...
  #runCommand(command: EditCommand) {
    ...
    switch (command) {
      ...
      case 'findNextMatch': {
        ...
        if (hasCollapsed) {
          const expanded: EditSelection[] = selections.map((sel) => {
      ...
      case 'indentMore':
        if (this.#selections !== undefined) {
          ...
          const nextSelections: EditSelection[] = [];
      ...
      case 'selectAll': {
        ...
        this.focus();
        // The edit paints selections with an overlay and otherwise leaves the
  ...
  #insertBlankLine(): void {
    ...
    const nextSelections = selections.map<EditSelection>((_, index) => {
  ...
  #rerender(
    ...
    // edit memoized for caret/selection geometry (#lineYCache,
    ...
    if (this.#options.__debug === true) {
      console.log(
        `[diffs/edit] re-render in: ${round(performance.now() - t2)}ms,`,
  ...
  #setWindowSelection(selection: EditSelection) {
    ...
    try {
      ...
      console.error('[diffs/edit] failed to update window selection:', err);
    }
  }
  ...
  #setDeletedTextSelectionActive(active: boolean): void {
    ...
    if (active) {
      ...
      // edit only clears them when its own selection range changes — but
  ...
  #setSelectedLinesSafe(
    ...
    try {
      ...
      // edit edits. Without it a split diff also highlights the matching
      ...
      // InteractionManager.renderSelection can throw while edit DOM is updating.
    }
  }
  ...
  #updateSelections(selections: EditSelection[]) {
    this.__postponeBgTokenizeToNextFrame();
  ...
  #updateSelectionActionPopover(): void {
    ...
    if (this.#selectionAction === undefined) {
      const getActiveSelection = (): EditSelection =>
  ...
  #applySelectionCutEdits(
    ...
    const nextSelections = nextSelectionOffsets.map<EditSelection>((offset) => {
  ...
  #applyChange(
    ...
    if (newSelections !== undefined) {
      // Always re-render the selection range and caret overlay so edit state
      // stays in sync. When skipFocus is set (a programmatic edit on an edit
      // that is not focused) we stop here: focusing or scrolling would pull the
      // caret and viewport toward an edit the user is not interacting with.
  ...
  // check if the web selection belongs to edit
  #rangeBelongsToEdit({ startContainer, endContainer }: StaticRange) {
    const contentEl = this.#contentElement;
