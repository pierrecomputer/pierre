...
export function renderDiffWithHighlighter(
  ...
  const baseThemeType =
    typeof options.theme === 'string'
      ? getThemeType(highlighter.getTheme(options.theme))
      : undefined;
...
function renderTwoFiles({
  ...
  const deletionLines =
    deletionFile.contents === ''
      ? []
      : renderTokenLines(
          highlighter.codeToTokens(cleanLastNewline(deletionFile.contents), {
            ...config,
            lang: deletionLang,
          }).tokens,
          deletionInfo,
  ...
  const additionLines =
    additionFile.contents === ''
      ? []
      : renderTokenLines(
          highlighter.codeToTokens(cleanLastNewline(additionFile.contents), {
            ...config,
            lang: additionLang,
          }).tokens,
          additionInfo,
