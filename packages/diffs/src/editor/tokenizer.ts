...
export class EditorTokenizer {
  ...
  #createTokenizer(
    ...
    this.#tokenizer = this.#highlighter.createLiveTokenizer({
      ...
      lang: this.#textDocument.languageId,
      theme: this.#theme,
  ...
  // Backend callbacks can run during tokenization. Deliver after the host has
  ...
  // Rebind system observers and reset tokens when the backend, theme, or
  // tokenization limit changes.
  syncTheme(
    codeOptions: BaseCodeOptions,
    highlighter: DiffsHighlighter = this.#highlighter
  ): void {
    if (this.#isCleanedUp) return;
    const highlighterChanged = highlighter !== this.#highlighter;
    this.#highlighter = highlighter;
    const {
      themeType = 'system',
    ...
    const resolved = this.#highlighter.getTheme(themeName);
    const appearance =
      typeof theme === 'string' ? (getThemeType(resolved) ?? 'dark') : mode;
    if (
      !highlighterChanged &&
    ...
    const colors = getThemeStyle(resolved);
  ...
  tokenize(
    ...
    if (this.#tokenizer === undefined) {
      ...
      // Complete only the prefix needed by this viewport, then read its cache.
      if (startingLine < endLine && this.#tokenizer.pendingTokenization)
        this.#tokenizer.flush(endLine);
      ...
      lines = (
        ...
        this.#textDocument.revision === this.#documentRevision + 1
          ? this.#tokenizer.tokenize(change, options)
          : this.#tokenizer.reset(options)
      ).lines;
    }
    this.#documentRevision = this.#textDocument.revision;
