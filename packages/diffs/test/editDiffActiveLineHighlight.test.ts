...
async function createDiffEditFixture(
  ...
  // element (edit.ts) in the same synchronous block that assigns
...
describe('editor active-line highlight on a diff', () => {
  ...
  test('split: a caret highlights only the additions column', async () => {
    const fixture = await createDiffEditFixture('split', OLD, NEW);
    try {
  ...
  test('unified: selecting a deleted line clears edit mode selection and toggles the deleted-text marker', async () => {
    ...
    try {
      ...
      // edit drops its own selection and the deleted-text marker turns on.
  ...
  test('unified: selecting a second deleted line clears the first one’s gutter highlight', async () => {
    ...
    try {
      ...
      // edit's own selection range, which is what normally clears these, so
