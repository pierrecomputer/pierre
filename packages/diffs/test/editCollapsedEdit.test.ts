...
// The edit attaches to the additions (new-file) side of a diff: the
...
async function createCollapsedEditFixture(): Promise<CollapsedEditFixture> {
  ...
  // Edit.edit's option forcing only runs once at attach.
...
function typeAt(
  ...
  edit.setSelections([{ start: position, end: position, direction: 'none' }]);
  edit.applyEdits(
...
describe('diff edit: attach-time option normalization', () => {
  test('the fallback setOptions preserves a host expandUnchanged option', async () => {
    const dom = installDom();
    const container = document.createElement('div');
    document.body.appendChild(container);
    // useTokenTransformer: false triggers Edit.edit's setOptions fallback,
...
describe('diff edit: collapsed regions during edit', () => {
  ...
  test('typing below a collapsed gap patches the row without duplicates', async () => {
    const fixture = await createCollapsedEditFixture();
    const { container, edit } = fixture;
    try {
      // Line 50 (index 49) sits below the collapsed gap.
      typeAt(fixture.edit, 49, 0, 'X');
      await wait(0);
      expect(edit.getFile()?.contents).toContain('Xline 50 changed');
  ...
  test('clicking a separator expand button mid-edit reveals rows and keeps edits', async () => {
    ...
    try {
      ...
      expect(edit.getFile()?.contents).toContain('EDIT-line 10 changed');
    } finally {
      await fixture.cleanup();
    }
  });
  ...
  test('typing into context revealed mid-edit lands correctly and undoes', async () => {
    const fixture = await createCollapsedEditFixture();
    const { container, edit, fileDiff } = fixture;
    try {
      ...
      // session region.
      typeAt(edit, 29, 0, 'typed ');
      await wait(30);

      expect(edit.getFile()?.contents).toContain('typed line 30');
      const rows =
      ...
      edit.undo();
      await wait(30);
      expect(edit.getFile()?.contents).not.toContain('typed line 30');
      const restored =
        findAdditionContent(container)!.querySelectorAll('[data-line="30"]');
  ...
  test('a line-count edit below the gap re-renders without duplicates', async () => {
    ...
    const { container, edit } = fixture;
    try {
      typeAt(edit, 49, 'line 50 changed'.length + 1, '\nNEW LINE');
      await wait(20);

      expect(edit.getFile()?.contents).toContain('line 50 changed\nNEW LINE');
      const lineNumbers = renderedLineNumbers(container);
      expect(new Set(lineNumbers).size).toBe(lineNumbers.length);
