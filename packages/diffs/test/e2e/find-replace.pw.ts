...
// Focus edit mode's surface, then open the search panel with the real shortcut.
...
test.describe('find and replace', () => {
  ...
  test('Escape from edit mode content closes the panel and clears highlights', async ({
    page,
  }) => {
    await openFixture(page);
    await openSearchPanel(page);
    ...
    // Click into edit mode's text so focus leaves the search input, then press
