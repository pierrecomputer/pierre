...
import { DirectionForward, DirectionNone } from '../src/edit/selection';
...
type EditableSelection = Parameters<
  Edit<undefined>['setSelections']
>[0][number];

interface CreateEditFixtureOptions {
  contents?: string;
  platform?: string;
...
async function createEditFixture(
  options: CreateEditFixtureOptions = {}
): Promise<EditFixture> {
...
async function openSearchReplacePanel({
  content,
  window,
}: Pick<EditFixture, 'content' | 'window'>): Promise<HTMLElement> {
...
describe('Edit composition input', () => {
  test('lets the browser own IME preview before committing composition text', async () => {
    ...
    try {
      ...
      expect(edit.getText()).toBe('line 1');
      ...
      expect(edit.getText()).toBe('line 1あ');
    } finally {
  ...
  test('survives selection changes when getComposedRanges is unavailable', async () => {
    // The edit renders inside a shadow root and reads the selection via
    ...
    const { cleanup, edit } = await createEditFixture();
...
describe('Edit keyboard editing', () => {
  ...
  test('uses primary Linux Ctrl+F as search instead of cursor move', async () => {
    const { cleanup, content, window } = await createEditFixture({
      contents: 'alpha beta',
      platform: 'Linux x86_64',
      selections: [
        {
  ...
  test('replaces the current match on Enter from the replace input', async () => {
    ...
    try {
      ...
      expect(event.defaultPrevented).toBe(true);
      expect(edit.getText()).toBe('omega beta alpha');
    } finally {
      cleanup();
    }
  });
  ...
  test('shifts later same-line carets past earlier inserted indents', async () => {
    ...
    try {
      ...
      expect(event.defaultPrevented).toBe(true);
      expect(edit.getText()).toBe('  abcde  fgh');
