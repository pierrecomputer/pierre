...
describe('WorkerPoolManager themes', () => {
  ...
  test('resolves worker themes through the retained highlighter after shared disposal', async () => {
    ...
    try {
      ...
      const retainedTheme = { ...retained.getTheme('github-dark'), name };
      ...
      const activeTheme = { ...retainedTheme, name, bg: '#ffffff' };
...
describe('WorkerPoolManager highlighters', () => {
  ...
  test.each(['highlights', 'shiki-wasm', 'shiki-js'] as const)(
    'preloads languages when initializing %s workers',
    async (preferredHighlighter) => {
      await disposeHighlighter();
      const langs = ['ts', 'text', 'unknown-language'];
      const { manager, worker } = await createInitializedManager({
        preferredHighlighter,
        langs,
      });
      try {
        ...
        expect(highlighter.hasLoadedLanguages?.(['typescript'])).toBe(
          preferredHighlighter === 'highlights' ? undefined : true
        );
        if (preferredHighlighter !== 'highlights') {
          expect(await worker.waitForInitializeRequest()).toMatchObject({
            langs,
          });
