...
async function createFixture(
  ...
  return {
    ...
    async toggleDisplayOption() {
      ...
      // Let syncRenderViewToEdit's highlighter promise resolve.
      await wait(10);
    },
    async cleanup() {
      await wait(10);
...
describe('diff edit: display-option toggle mid-edit', () => {
  test('keeps the edited line text visible when a display option is toggled', async () => {
    ...
    try {
      typeAt(edit, 0, 5, 'X');
      await wait(0);
      expect(edit.getText()).toBe('alphaX\nCHANGED\n');
  ...
  test('accepts further typing after the toggle without duplicating the edit', async () => {
    ...
    try {
      ...
      expect(edit.getText()).toBe('alphaXY\nCHANGED\n');
...
describe('diff edit: detach then re-attach', () => {
  ...
  test('renders edits typed after a detach/re-attach cycle', async () => {
    ...
    try {
      ...
      // Detach (Edit-mode off) then re-attach the same edit (Edit-mode on).
      edit.cleanUp();
      await wait(0);
      edit.edit(fileDiff);
  ...
  test('ignores a stale async edit sync after reset remounts the diff', async () => {
    const dom = installDom();
    const container = document.createElement('div');
    document.body.appendChild(container);
    ...
    try {
      ...
      // Queue an edit sync from the edited instance, then reset by unmounting
      // that instance and mounting a fresh diff with the original contents.
      (
        fileDiff as unknown as { syncRenderViewToEdit(): void }
      ).syncRenderViewToEdit();
      fileDiff.cleanUp();
