...
describe('Edit edits at the bottom of a virtualized window', () => {
  // A run of Enter presses at the window's bottom edge must keep rendering each
  // new line. The edit widens its render range when the caret reaches the
  ...
  test('keeps consecutive newlines at the window bottom rendered', async () => {
    const { cleanup, content, edit, fileContainer } = await createWindowedEdit(
      200,
      makeRange(100, 50)
    );
    try {
      ...
      edit.setSelections([collapsedCaret(149)]);
      edit.applyEdits([
        {
          range: {
  ...
  // virtualization. The edit caps widening at twice the synced window (here
  ...
  test('keeps consecutive inserts from materializing unbounded rows', async () => {
    ...
    try {
      ...
      for (let round = 0; round < 40; round++) {
        edit.setSelections([collapsedCaret(caret)]);
        edit.applyEdits([
          {
            range: {
  ...
  test('does not widen for an edit that starts below the rendered window', async () => {
    ...
    try {
      ...
      edit.setSelections([collapsedCaret(180)]);
      edit.applyEdits([
        {
          range: {
