...
function expectTokens(
  ...
  const expected = codeToTokens(document.getText().replace(/\r(?!\n)/g, '\n'), {
...
describe('EditorTokenizer', () => {
  ...
  test('undo and redo retain tokens when an inserted LF joins an existing CR', () => {
    const instance = create('a\rb');
    instance.initialize();
    expectTokens(instance.document, instance.update([edit(1, 0, 1, 0, '\n')]));
    for (const action of ['undo', 'redo'] as const) {
      const [change] = instance.document[action]()!;
      const lines = instance.tokenizer.tokenize(change);
      expect([...lines.keys()]).toEqual([0, 1]);
      expectTokens(instance.document, lines);
    }
