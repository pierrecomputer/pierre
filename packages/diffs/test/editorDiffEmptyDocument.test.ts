...
function replaceAll(editor: Editor<undefined>, newText: string): void {
  const lines = editor.getContent()!.contents.split('\n');
  ...
  for (const diffStyle of ['split', 'unified'] as const) {
    test(`keeps an editable line, accepts typing, and undoes (${diffStyle})`, async () => {
      ...
      try {
        ...
        replaceAll(editor, '');
        await wait(0);
        expect(editor.getContent()!.contents).toBe('');
        ...
        await wait(0);
        expect(editor.getContent()!.contents).toBe('hello');

        // Undo reverts the typing, then the deletion, back to the original.
        editor.undo();
        editor.undo();
        for (let attempt = 0; attempt < 40; attempt++) {
          if (editor.getContent()!.contents === 'a\nb\nc\n') {
            break;
          }
          await wait(10);
        }
        expect(editor.getContent()!.contents).toBe('a\nb\nc\n');
      } finally {
