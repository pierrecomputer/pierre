...
import { findBracketMatchRanges } from '../src/edit/matchBrackets';
import { TextDocument } from '../src/edit/textDocument';
import { EditTokenizer } from '../src/edit/tokenzier';
...
describe('editor bracket matching', () => {
  ...
  test('does not render bracket matches when disabled', async () => {
    const { cleanup, content, edit } = await createBracketMatchFixture('a(b)', {
      matchBrackets: false,
    });
    try {
      edit.setSelections([
  ...
  test('does not read ignored token ranges when disabled', async () => {
    const getIgnoredRangesSpy = spyOn(
      EditTokenizer.prototype,
      'getStringCommentRegexpRangesInLine'
    );
    const { cleanup, edit } = await createBracketMatchFixture('{ /{/ }', {
      matchBrackets: false,
    });
    try {
      edit.setSelections([
  ...
  test('ignores brackets inside quoted strings and template literals', async () => {
    const { cleanup, content, edit } = await createBracketMatchFixture(
      '[ \'abc[\' ]\n[ "abc[" ]\n[ `abc[` ]'
    );
    try {
