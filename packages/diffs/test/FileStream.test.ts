...
describe('FileStream', () => {
  ...
  test('shows unfinished chunks and replaces them with native highlighted tokens', async () => {
    const stream = await create('highlights');
  ...
  test('queued setup resolves the latest backend, theme, and language', async () => {
    ...
    const originalLoader = highlighters[defaultHighlighter];
    let release!: () => void;
    ...
    highlighters[defaultHighlighter] = async () => {
      signalStarted();
      await paused;
    ...
    try {
      ...
      release();
      highlighters[defaultHighlighter] = originalLoader;
  ...
  test('setup can retry after a failed backend load', async () => {
    ...
    highlighters[defaultHighlighter] = () => Promise.reject(loadError);
    try {
