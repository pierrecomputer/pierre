...
import { isMoveCursorShortcut } from '../src/edit/platform';
...
function setCaret(edit: Edit<undefined>, line: number, character = 0) {
...
describe('diff edit: fold-skip navigation', () => {
  test('page keys are not edit-handled cursor shortcuts', () => {
  ...
  test('arrow-up at a hunk boundary skips the collapsed gap', async () => {
    ...
    try {
      ...
      expect(edit.getState().selections?.at(-1)?.start.line).toBe(13);
    } finally {
      await fixture.cleanup();
    }
  });
  ...
  test('arrow-right at the end of a hunk boundary line skips the gap', async () => {
    ...
    try {
      ...
      setCaret(edit, 13, 'line 14'.length);
      await wait(0);
      pressKey(content, 'ArrowRight');
      const selection = edit.getState().selections?.at(-1);
  ...
  test('horizontal motion stays put when only collapsed lines remain', async () => {
    ...
    try {
      ...
      setCaret(edit, 53, 'line 54'.length);
      await wait(0);
      pressKey(content, 'ArrowRight');
      let selection = edit.getState().selections?.at(-1);
      ...
      await wait(0);
      pressKey(content, 'ArrowLeft');
      selection = edit.getState().selections?.at(-1);
...
describe('diff edit: reveal-on-jump', () => {
  ...
  test('undo whose caret restore lands in a collapsed region expands it', async () => {
    ...
    try {
      ...
      edit.undo();
      await wait(20);
      expect(edit.getFile()?.contents).not.toContain('edited line 30');
