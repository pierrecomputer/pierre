...
import { PopoverManager } from '../src/edit/popover';
import { DirectionBackward, getCaretPosition } from '../src/edit/selection';
import type { SelectionActionContext } from '../src/edit/selectionAction';
...
// edit's overlay layer as soon as a ranged selection settles.
...
describe('Edit selection action', () => {
  ...
  test('forward-grown selection: acts on the full selection, not the first character', async () => {
    ...
    const { cleanup, edit, content } = await createSelectionActionFixture(
      'hello world',
      {
    ...
    try {
      ...
      expect(edit.getText()).toBe('TODO(hello) world');
    } finally {
      cleanup();
    }
  });
  ...
  test('recomputes popover placement when selection action content resizes', async () => {
    ...
    const { cleanup, edit, content, triggerResizeObserver } =
  ...
  test('refocus keeps a backward selection backward', async () => {
    ...
    try {
      ...
      expect(edit.getState().selections?.[0]?.direction).toBe(
        DirectionBackward
      );
      ...
      const primarySelection = edit.getState().selections?.at(-1);
  ...
  test('composition-time selectionchange does not affect a backward selection', async () => {
    const { cleanup, edit, content, window } =
