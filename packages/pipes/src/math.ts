...
export function mat4FromBasis(out: Mat4, x: Vec3, y: Vec3, z: Vec3, origin: Vec3): Mat4 {
...
export function mat4ScaleTranslate(out: Mat4, sx: number, sy: number, sz: number, tx: number, ty: number, tz: number): Mat4 {
  out.fill(0);
  out[0] = sx;
  out[5] = sy;
  out[10] = sz;
