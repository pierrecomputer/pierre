...
The classic Windows "3D Pipes" screensaver, recreated from scratch in raw WebGL2.
No dependencies — hand-written matrix math, procedural geometry, and shaders in
~15 kB (5 kB gzipped).
...
formula — one white directional light in eye space, full-strength ambient
light model, per-material specular, no gamma correction — on pure black.
...
- **The camera never enters the pipe volume**: fixed eye distance, wide 90°
  FOV, grid height scaled to the window aspect ratio, and each round rotates
  the scene a further 9.73156° around Y.
...
```sh
npm install canvas-pipes
```

## Usage

```ts
import { createPipes } from 'canvas-pipes';
...
The canvas is sized automatically from its CSS layout size (via `ResizeObserver`),
so style it however you like — fullscreen, in a card, wherever pipes belong.
...
createPipes(canvas, {
  ...
  materials: [ /* full PipeMaterial entries; defaults to CLASSIC_MATERIALS */ ],
});
```
...
```sh
npm install
npm run dev     # demo page at http://localhost:5173 (press "r" to force a reset)
npm run build   # library build + type declarations into dist/
```
