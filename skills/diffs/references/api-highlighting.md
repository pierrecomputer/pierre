...
`registerCustomTheme(name, loader, type?)` accepts `'zed'` for Highlights or
`'textmate'` for either Shiki backend. Register the same name in each format to
switch backends. Without `type`, the loader is registered for all backends; its
result must match the selected backend's format.
...
custom loaders must return that same format. The default provider is Highlights.
