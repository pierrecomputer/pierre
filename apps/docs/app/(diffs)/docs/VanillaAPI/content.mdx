...
`File` (render a single code file without diff), and `UnresolvedFile` (render
merge conflicts with built-in resolution controls). Typically you'll want to
interface with these as they'll handle the complicated aspects of syntax
highlighting, theming, and interactivity for you.
