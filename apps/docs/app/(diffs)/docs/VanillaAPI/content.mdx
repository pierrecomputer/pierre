...
See [Edit mode → Vanilla JS](#edit-mode-vanilla-js) for attaching `Editor` to a
...
Editable surfaces take `onEditChange` (the live change stream) and
`onEditComplete` (the accept/revert boundary) as `FileOptions` /
`FileDiffOptions` members. Edit mode owns annotation positions during a session,
so you do not sync them back; accepting installs the final collection. See
[Completing an edit](#edit-mode-completing-an-edit) and
