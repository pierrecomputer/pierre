...
The Vanilla JS API exposes four core components: `CodeViewer` (render a mixed,
...
<VanillaComponentTabs
  codeViewerExample={codeViewerExample}
...
The `CodeViewer` tab above is the quick-start version. For the deeper guide on
...
`CodeViewer` forwards many of those same options to each rendered item, while
...
[CodeViewer](#code-viewer) for the dedicated guide.
