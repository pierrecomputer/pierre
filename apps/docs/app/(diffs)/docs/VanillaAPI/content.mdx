...
The Vanilla JS API exposes five core components: `CodeView` (render a mixed,
...
`File` (render a single code file without a diff), `CodeEditor` (render one
editable, virtualized file with edit mode built in), and `UnresolvedFile`
(render merge conflicts with built-in resolution controls). Start with these
components for syntax highlighting, theming, layout, and interactivity.

<VanillaComponentTabs
  codeViewExample={codeViewExample}
  fileDiffExample={fileDiffExample}
  fileExample={fileExample}
  codeEditorExample={codeEditorExample}
  unresolvedFileExample={unresolvedFileExample}
/>

> `CodeEditor` and `UnresolvedFile` are currently beta/experimental and may
> change in future releases.

See [Edit mode → Vanilla JS](#edit-mode-vanilla-js) for `CodeEditor` `render` /
`setFile`, placeholders, and its relationship to the lower-level `Editor` API.
