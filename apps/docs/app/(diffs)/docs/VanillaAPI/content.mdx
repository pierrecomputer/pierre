...
See [Edit mode → Vanilla JS](#edit-mode-enable-editing-vanilla-js) for attaching
...
`onEditComplete` (the accept/reject boundary when a session ends) as
...
constructor options. It is required whenever the component needs full file
contents: `expandUnchanged`, `expandHunk`, or editing. Without it those paths
throw. The loader receives the partial `FileDiffMetadata` and returns
`{ oldFile, newFile }`: changed and rename-changed diffs return both sides,
while pure renames return `{ oldFile: null, newFile }`. Added and deleted patch
diffs do not need loader hydration and cannot be edited from a patch. Components
catch loader errors by default; set `disableErrorHandling: true` when you want
errors to rethrow.
...
`createEditor(editorType, options, editStateKey)` and optional
`getEditStateKey(item)`. See [CodeView](#codeview) for the dedicated guide and
[Retain edit state in memory](#edit-mode-retain-edit-state-in-memory) for state
