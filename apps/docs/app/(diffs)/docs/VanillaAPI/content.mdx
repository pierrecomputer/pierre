...
output as lightweight HTML tree nodes (`HElement`, serializable with `toHtml`)
or HTML strings for custom rendering pipelines.

#### DiffHunksRenderer

Takes a `FileDiffMetadata` data structure and renders out the raw HTML tree
elements for diff hunks. You can generate `FileDiffMetadata` via
`parseDiffFromFile` or `parsePatchFiles` utility functions.
...
syntax-highlighted code as HTML tree elements. Useful for rendering single files
