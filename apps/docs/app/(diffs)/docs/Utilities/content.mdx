...
Release the shared highlighter reference so the next request creates an instance
with its own theme cache. Loaders registered through `registerCustomTheme` carry
over to the new instance. Existing highlighter references keep their themes and
...
Get direct access to the shared Highlights highlighter instance used internally
by all components. Useful for custom highlighting operations.
...
Preload themes before rendering to ensure instant highlighting with no async
loading delay.
...
Register a custom Highlights theme for use with any component. The theme name
you register must match the `name` field inside your theme JSON file.

<DocsCodeExample {...registerCustomTheme} />

Languages are bundled with Highlights. Set `file.lang` to a supported language
or use `setLanguageOverride` to assign one to a file or diff. Unsupported file
