...
Clear the shared highlighter instances for every backend so the next render
creates fresh ones. Useful when cleaning up resources in single-page
applications. Custom theme and language registrations are kept.

<DocsCodeExample {...disposeHighlighter} />

### getSharedHighlighter [toc-ignore]

Get direct access to the shared highlighter instance for a backend
(`'shiki-wasm'` by default), the same instance components use internally. Pass
the `themes` to resolve, optional Shiki `langs` to load, and
`preferredHighlighter`. The instance exposes `codeToTokens`, stream and live
tokenizers, `getTheme`, and a theme resolver for custom highlighting operations.
...
highlighting with no async loading delay. Set `preferredHighlighter` to preload
a specific backend; `'highlights'` bundles its languages and ignores `langs`.

<DocsCodeExample {...preloadHighlighter} />

### registerCustomTheme [toc-ignore]

Register a custom theme for use with any component. Themes are registered per
format: pass `'textmate'` as the third argument for the Shiki backends
(`'shiki-wasm'` and `'shiki-js'`) or `'zed'` for Highlights. Omit it to register
the loader for every backend, in which case the loader must return the format of
whichever backend loads it. The registered name is applied to the theme even
when the loaded object declares a different `name`. Duplicate names within a
format are logged and ignored.
...
want custom highlighting grammars. Both Shiki backends load the grammar on
demand, and it is forwarded to Shiki worker pools. The Highlights backend uses
