...
A partial diff item in edit mode starts loading its full files when it renders
(in view, not collapsed). If it scrolls out during the load, CodeView keeps the
result and primes its highlighting without rendering the item. The editor
attaches once a load completes while the item is rendered. Until then
`getEditor(itemId)` returns `undefined`, and `onAttach` runs after the editor's
first render sync. A partial edit item without `loadDiffFiles` throws from
`render()` once it renders.
