...
the owning item. `onItemEditComplete(event, item, nextItem)` is the
accept/revert boundary: when a changed session ends (editing disabled, or the
item removed), return `'accept'` to install the edit — `CodeView` applies the
built `nextItem`, which a controlled owner also mirrors into its own state — or
`'reject'` to revert. Collapsing an item suspends its session rather than
completing it. A direct `reset()` or `cleanUp()`, and viewer unmount, would also
complete changed sessions.

Each edited item receives an independent editor whose document, history,
selections, and editor-owned view state survive virtualization. Changes to the
...
Use `getEditHistoryKey(item)` to retain that complete editor state after its
edit session ends. React passes the resolved key through its `EditProvider`;
vanilla `CodeView` passes it to `createEditor` as the third argument. CodeView
does not derive this key from `item.id` or a render cache key, so choose a
stable application identity explicitly. Retention is bounded and in-memory only.
...
through on `onItemEditChange`. `event.lineAnnotations` on that callback reports
the current collection if you want to observe it. You only write
`item.annotations` (with a `version` bump) when you genuinely add/remove/update
existing annotations.

Currently if you add/remove/update any annotations while in an edit session, it
will revert any positional edit changes because there's currently no way to
resolve changes. You may want to disable external annotation updates while
editing and figure out how to resolve them post edit.
