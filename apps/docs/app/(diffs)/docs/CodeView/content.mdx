...
be editable, and pass creation-time item-editor behavior through `editOptions`.
Use `onItemEditChange` for live changes and `onItemEditComplete` for completed
sessions; both callbacks include the owning item.

Each edited item receives an independent editor. That editor and its history
survive when virtualization moves the item out of and back into the rendered
window. Turning `edit` off, collapsing or removing the item, or unmounting the
viewer ends the session and cleans the editor. Changes to the provider factory
or `editOptions` do not disturb active sessions; their latest values apply the
next time an item enters edit mode. Use `getEditor(id)` for item-specific
history, selection, marker, save, and search controls.
