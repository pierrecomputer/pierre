...
Use `getEditHistoryKey(item)` to retain an item's editable draft and undo/redo
history after its edit session ends. React passes the resolved key through its
`EditProvider`; vanilla `CodeView` passes it to `createEditor` as the third
argument. CodeView does not derive this key from `item.id` or a render cache
key, so choose a stable application identity explicitly. Retention is bounded
and in-memory only.
