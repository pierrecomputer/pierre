...
uses [Highlights](/highlights) by default. Set `preferredHighlighter` to
`'shiki-wasm'` or `'shiki-js'` in component options to use Shiki. With a
[Worker Pool](#worker-pool), set this option on the pool instead. Each backend
loads on demand and supports file, diff, editor, and streaming highlighting.
