...
VS Code Marketplace or Open VSX. The generated Zed theme data can also be used
with `@pierre/diffs`.

### Using with Diffs

Diffs uses Highlights by default, which accepts Zed themes with `name`,
`appearance`, and `style`. Register a loader with
`registerCustomTheme(name, loader, 'zed')`. The `'shiki-wasm'` and `'shiki-js'`
backends accept TextMate themes with `name`, `type`, `colors`, and
`tokenColors`; use `registerCustomTheme(name, loader, 'textmate')`.

Register the same name in both formats to switch backends without changing the
`theme` option. When loading a Zed theme family, select one member from its
`themes` array. Loaders run when their themes are first requested:
...
<Notice variant="warning" icon={<IconCiWarningFill />}>
  ...
  Highlights and Shiki on the web, using the matching format for each.
</Notice>
