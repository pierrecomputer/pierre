...
Shiki, `'zed'` for Highlights, and `'diffs'` for portable `DiffsTheme` objects.
A Zed family uses its first theme; return another member from the loader to
select it. The registered name replaces the Zed theme's display name.
...
register its loader with `'diffs'`. `registerCustomCSSVariableTheme` does this
automatically.
