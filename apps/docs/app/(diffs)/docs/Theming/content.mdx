...
Diffs uses Highlights themes, which follow the Zed format. Register a theme
object with `name`, `appearance`, and `style` before rendering. When loading a
Zed theme family, select one member from its `themes` array:
