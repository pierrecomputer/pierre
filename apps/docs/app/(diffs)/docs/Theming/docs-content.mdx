...
While you can use any bundled theme with Pierre Diffs by passing its name to the
`theme` option, you can also create and register custom themes: TextMate-format
themes (Shiki and Visual Studio Code) for the Shiki backends, or Zed-format
themes for Highlights. We recommend using our themes as a starting point for
your own custom themes—head to our [themes documentation](/theme) to get
started.
