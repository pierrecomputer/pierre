...
Use any bundled Highlights theme with Pierre Diffs by passing its name to the
`theme` option. Custom themes use the Highlights `Theme` format: a `name`,
`appearance`, and `style` with syntax colors. We recommend using our themes as a
starting point for your own custom themes—head to our
[themes documentation](/theme) to get started.
