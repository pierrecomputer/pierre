...
`editor.cleanUp('discard')` also runs completion for changed content, but never
...
Completion runs only when the final contents differ from the latest external
...
Unmounting a changed component or item can still run completion, but does not
install the result. The callback remains the place to capture or persist that
final content.
