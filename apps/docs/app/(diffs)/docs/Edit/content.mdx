...
aborts in-progress work through `context.signal`. For `'subtle'` mode, reveal a
prediction with <kbd>Alt</kbd> before accepting or dismissing it. Use `include`
and `exclude` to filter slash-normalized file paths; exclusions take precedence.
