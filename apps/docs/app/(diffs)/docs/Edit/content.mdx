...
- **`createEditor`** — a factory called whenever a surface starts an edit
  session. Each editable `File`, `FileDiff`, `MultiFileDiff`, or `PatchDiff`
  surface and each editable `CodeView` item gets its own editor, so state stays
  isolated: sibling surfaces have independent history, selections, and
  callbacks, and re-entering edit mode starts fresh.
