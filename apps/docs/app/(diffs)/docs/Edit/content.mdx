...
The provider supplies editors in one of two modes:

- **`createEditor`** — a factory called when a surface starts an edit session.
  Each editable `File`, `FileDiff`, `MultiFileDiff`, or `PatchDiff` surface and
  each editable `CodeView` item gets its own editor, so state stays isolated:
  sibling surfaces have independent history, selections, and callbacks. Created
  editors are cached by `editorOptions` object identity — re-entering edit mode
  with the same options object reuses the surface's editor; pass a new object to
  start fresh.
- **`sharedEditor`** — one `Editor` instance handed to every surface under the
  provider, exactly as you constructed it. This is the mode for IDE-like UIs
  that edit one file at a time across many files, normally combined with
  `persistState` so each file's contents, history, selections, and scroll
  position survive switches — see
  [Persisting Editor State](#edit-mode-persisting-editor-state). When both props
  are set, `sharedEditor` wins.

With `createEditor`, pass creation-time callbacks and behavior through the
surface's `editorOptions`; the factory can spread shared defaults before the
surface options so the surface wins. Keep factories and object props stable:
define static values at module scope, or use `useCallback` and `useMemo` when
they depend on component values.

<DocsCodeExample {...editReactCreateEditorExample} />

With `sharedEditor`, the surfaces' `editorOptions` props are ignored — configure
the editor at construction and reuse the instance:
...
- **Editor state.** Selections and scroll positions — the code scroller's
  ...
  on the next attach with the same `cacheKey`.
...
In React, hand that one editor to every surface with the `EditProvider`
`sharedEditor` prop. The surfaces' `editorOptions` props are ignored in this
mode — configure the editor at construction:
