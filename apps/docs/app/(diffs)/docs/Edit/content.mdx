...
4. **Updates** — Keystrokes update an internal `TextDocument`, then changed
   ...
   mode. Your `onChange` handler receives an event with normalized text
   `changes`, updated `FileContents` in `file`, and `lineAnnotations` when
   present.
...
remaps them and passes the complete current collection as `onChange`'s
`lineAnnotations` alongside the edited `file`. This collection is authoritative
when present; it is not a delta. Replace your application-owned collection with
it before a later render can reapply stale coordinates.
...
The shared `onChange` event accepts either annotation shape: standalone `File`
surfaces emit `LineAnnotation[]`, while diff surfaces emit
...
Use `onAttach` to opt into initial caret placement. Its event provides the
attached `editor` and `fileInstance`; at this point the text document and
editable DOM are ready. In React, pass a stable `editorOptions` object to any
editable surface, including `CodeView`:
