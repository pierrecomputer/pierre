...
response as ghost text and applies it when the user presses <kbd>Tab</kbd>. Keep
model credentials in your service rather than shipping them to the browser.
...
aborts in-progress work through `context.signal`. In `'subtle'` mode, hold

<kbd>Alt</kbd> to reveal a prediction before accepting it with <kbd>Tab</kbd>.
Use `include` and `exclude` to filter slash-normalized file paths; exclusions
take precedence.
