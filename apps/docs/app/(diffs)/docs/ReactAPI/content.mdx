...
The React API exposes seven main components:

- `CodeView` renders a mixed, virtualized list of files and diffs inside one
  scroll container
- `CodeEditor` renders one editable, virtualized file with edit mode built in
...
<ComponentTabs
  ...
  reactAPICodeEditor={reactAPICodeEditor}
  reactAPIUnresolvedFile={reactAPIUnresolvedFile}
/>

See [Edit mode → React](#edit-mode-react) for `CodeEditor` options,
placeholders, refs, and its relationship to the lower-level `Editor` API.
