...
[Edit mode → React](#edit-mode-react) and
...
Every editable surface (`File`, `FileDiff`, `MultiFileDiff`, `PatchDiff`) also
takes `onEditChange` — the per-surface live change stream — and `onEditComplete`
— the one-time accept/revert boundary. Edit mode owns annotation positions
during a session, so you do not sync them back; accepting installs the final
collection. See [Completing an edit](#edit-mode-completing-an-edit) and
