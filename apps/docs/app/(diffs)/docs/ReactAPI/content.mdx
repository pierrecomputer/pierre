...
- `CodeViewer` renders a mixed, virtualized list of files and diffs inside one
  scroll container
...
<ComponentTabs
  reactAPICodeViewer={reactAPICodeViewer}
...
The `CodeViewer` tab above is the quick-start version. For the full guide on
items, ids, `version`, selection, and `scrollTo`, see
...
`CodeViewer` reuses many of the same option names internally, but it has its own
`items`, viewer ref, and mixed-item render props. See [CodeViewer](#code-viewer)
