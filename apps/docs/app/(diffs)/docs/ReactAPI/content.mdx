...
surfaces use `edit` and `editOptions`; `CodeView` uses item `edit` flags and its
own `editOptions`. Each active surface or item receives an independent editor.
`UnresolvedFile` is not editable. See [Edit mode → React](#edit-mode-react) and
