...
See [Edit mode → React](#edit-mode-react) for attaching `Editor` via
`EditProvider`, standalone `edit` props or `CodeView` item flags, and
creation-time `editOptions`.

`EditProvider` accepts a stable `createEditor(options)` factory. Editable
`File`, `FileDiff`, `MultiFileDiff`, and `PatchDiff` surfaces pass their
creation-time `editOptions` into that factory at the start of each edit session.
React `CodeView` uses the same provider and passes its `editOptions` plus an
item-routing change callback whenever an item enters edit mode. Each surface or
item receives an independent editor. `UnresolvedFile` is not editable and does
not accept either prop.
...
functions. This applies to `options`, `editOptions`, annotations, and render
