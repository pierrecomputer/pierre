...
The loader returns the same file-side shape used by `MultiFileDiff`:
`{ oldFile, newFile }`, where each side is `FileContents` when it exists or
`null` when it intentionally does not.
