...
Pass an optional `highlighter` alongside `file` and `options` to choose the
implementation for that server render. For example,
`preloadFile({ file, highlighter: highlightsHighlighter })` uses Highlights
without changing the process-wide registration. The highlighter is omitted from
the returned component props. Configure the client to use the same
implementation for hydration. See
[Highlights server rendering](#highlights-server-rendering) for an example.
...
Preloads a diff directly from file contents. This is the simplest option when
...
Low-level renderers return HTML rows and provide `renderFullHTML(result)` to
compose their wrappers. `renderHTML(children)` from `@pierre/diffs/ssr` accepts
an array of rendered HTML strings and prepends the icon sprite used by Diffs.
