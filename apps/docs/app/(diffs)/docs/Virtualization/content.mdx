...
- Use `resizeDebugging` with the `Virtualizer` temporarily when tuning metrics,
  ...
  production.
- While in edit mode, annotations (both their visibility and positions) will be
  managed for you. Do not feed the onChange results back into component state.
  See [Line annotations](#edit-mode-extend-the-editing-ui-line-annotations).
