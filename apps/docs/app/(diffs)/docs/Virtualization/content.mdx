...
If your scrollable region is only code, start with `CodeViewer` instead. It is
the more optimized path: it owns the entire code surface, only renders what you
can actually see, and is generally more performant and less prone to blanking.
...
harder for `CodeViewer` to control. That flexibility comes with tradeoffs: every
