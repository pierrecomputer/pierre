...
The editor is a pluggable editing layer for `File`-style code surfaces.
It adds keyboard-driven editing to an already-rendered `File` or `FileDiff` while the
existing renderer continues to own syntax highlighting, layout, annotations, and
virtualization.

To attach the editor to a `File` instance, import it from `@pierre/diffs/editor`. It is intentionally separate from the
...
For vanilla integrations, render with `File` or `FileDiff` first, then attach the editor.
For best performance when editing large files, we strongly recommend using `VirtualizedFile` and `VirtualizedFileDiff`.
...
recommend using the `Virtualizer` component for best performance when editing large files.
...
`renderQuickEdit`, then click the gutter icon on the active selection to open it.
