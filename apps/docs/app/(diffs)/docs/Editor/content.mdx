...
`EditProvider`, and set `contentEditable` on `File`, `FileDiff`,
`MultiFileDiff`, or `PatchDiff`. The provider cleans up on unmount. Use
`Virtualizer` for large editable files. A common single-file pattern is:
