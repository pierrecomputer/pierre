...
Use the `Editor` API to add editing to a rendered `File`, `FileDiff`,
`MultiFileDiff`, or `PatchDiff`. In React, wrap the surface in `EditProvider`
and set `contentEditable`. In vanilla JS, call `editor.edit(component)` after
render.
...
1. **Attach** — Call `editor.edit(component)` in vanilla JS, or pass
   ...
   `PatchDiff` wrapped in `EditProvider`. The hookup returns a dispose function
   (vanilla) or runs automatically when `contentEditable` toggles (React).
...
Create one `Editor` instance (typically with `useMemo` or `useStableEditor`),
wrap your tree in `EditProvider`, and set `contentEditable` on `File`,
`FileDiff`, `MultiFileDiff`, or `PatchDiff`. The provider cleans up on unmount.
Use `Virtualizer` for large editable files. A common single-file pattern is:

```tsx
<EditProvider editor={editor}>
  <Virtualizer>
    <File file={file} contentEditable />
  </Virtualizer>
</EditProvider>
```
