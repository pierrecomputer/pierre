...
defaults to `true`; passing `false` omits selection restoration metadata but
keeps the text edit undoable. Use `editor.canUndo` and `editor.canRedo` to
...
keyboard shortcuts (<kbd>Cmd/Ctrl-Z</kbd> and
