...
For one editable file, start with `CodeEditor` in the [React](#edit-mode-react)
or [Vanilla JS](#edit-mode-vanilla-js) guide. Use the lower-level `Editor` API
when you need to add editing to an existing `File`, `FileDiff`, `MultiFileDiff`,
or `PatchDiff`.
...
For one editable file, use `CodeEditor` from `@pierre/diffs/react`. It combines
a scroll container, virtualized `File`, and attached `Editor`, so you do not
need to wire `Virtualizer`, `File`, and `EditorProvider` yourself. Pass `file`,
editor callbacks (`onChange`, `onAttach`), and file options such as `theme` as
top-level props. Use `ref` to reach the underlying `Editor` for
[`getState` / `setState`](#edit-mode-api-reference) and programmatic selections.
Omit `file` to show `renderPlaceholder`.

To edit an existing surface, create one `Editor` instance (typically with
`useMemo`), wrap your tree in `EditorProvider`, and set `contentEditable` on
`File`, `FileDiff`, `MultiFileDiff`, or `PatchDiff`. The provider cleans up on
unmount. Use `Virtualizer` for large editable files.

<EditorComponentTabs
  codeEditorExample={editorCodeEditorReactExample}
...
For one editable file, use `CodeEditor` from `@pierre/diffs`. Construct
`new CodeEditor(options)`, then call `render(root, file?)`, `setFile`, or
`setLineAnnotations` as needed. Call `cleanUp()` when the editor is removed.
Omit `file` on `render` to show `renderPlaceholder`.

For other surfaces, render with `File` or `FileDiff` first, then attach the
editor. Use `VirtualizedFile` and `VirtualizedFileDiff` for large editable
files.

<EditorComponentTabs
  codeEditorExample={editorCodeEditorVanillaExample}
...
`CodeEditor` options combine the `Editor` options above with selected `File`
options, plus `overscrollSize`, `renderAnnotation`, and `renderPlaceholder`. In
React, pass them as top-level props; in vanilla, pass them to the constructor.

<DocsCodeExample {...editorCodeEditorOptionsType} />

### API Reference

These methods are available on an `Editor` instance. Attach it to a rendered
`File` or `FileDiff` with `edit()` first, or use `CodeEditor` to manage that
step for you. `CodeEditor` inherits the full `Editor` API and adds `render`,
`setFile`, and `setLineAnnotations`.
...
You can drive the editor from code instead of relying solely on user input.
`editor.setSelections(selections)` replaces the current selections—pass more
than one for multiple cursors—and `editor.focus()` moves focus into the editor,
placing the caret at the primary selection.
