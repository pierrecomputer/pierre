...
<Notice icon={<IconInfoFill />}>
  `CodeViewer` is the high-level API for rendering one large scroll region that
  can contain files, diffs, or a mix of both.
</Notice>

`CodeViewer` takes an ordered `CodeViewerItem[]` list and manages the hard parts
...
- If you keep the same item id but change its content or annotations, publish a
  new `version` so `CodeViewer` refreshes that item in place without rebuilding
  the whole surface.
...
- Viewer-level options such as `viewerMetrics`, `itemMetrics`, `stickyHeaders`,
  ...
  forwarded to each item, but `collapsed` is item-owned in `CodeViewer`.

### Examples

<CodeViewerExampleTabs
  reactExample={codeViewerReactExample}
  vanillaExample={codeViewerVanillaExample}
/>

### Usage Notes

- In React, `CodeViewer` is controlled through props plus an imperative ref.
...
- `renderCustomHeader`, `renderHeaderPrefix`, `renderHeaderMetadata`,
  ...
  `CodeViewerItem`, which makes it easy to branch on `item.type`.
- In Vanilla JS, `CodeViewer` owns a scrollable root that you set up once and
  update over time.
...
- By default, `CodeViewer` temporarily disables pointer events on rendered
  content while scrolling for smoother scroll performance. Set
...
If your scrollable region is only code, `CodeViewer` should usually be your
...
mixed-content layout that `CodeViewer` cannot own directly. That flexibility
comes with tradeoffs: the lower-level virtualizer always mounts every top-level
file or diff container, is easier to blank during aggressive scroll, and is
usually less performant than `CodeViewer`.
