...
export function HistoryDemo({ prerenderedFile }: HistoryDemoProps) {
  ...
  const applyEdit = useCallback(
    (content: HTMLElement, index: number) => {
      const step = HISTORY_DEMO_EDITS[index];
      const text = SNAPSHOTS[index];
      const at = text.indexOf(step.find);
      if (at < 0) {
        return;
      }
      edit.setSelections([
        {
          ...
          end: offsetToPosition(text, at + step.find.length),
          direction: 'forward',
        },
      ]);
      content.dispatchEvent(
  ...
  const reset = useCallback(() => {
    ...
    preserveScroll(() => {
      let guard = 0;
      while (edit.canUndo && guard < 1000) {
        edit.undo();
        guard += 1;
      }
      seedAll(content);
    });
  }, [edit, getContent, preserveScroll, seedAll]);
