...
Provide `onMergeConflictAction` for controlled state (for example, to persist
resolved contents, sync external stores, or trigger side effects). Use
`onMergeConflictResolve` when you want uncontrolled resolution plus a
notification with the resolved file.
