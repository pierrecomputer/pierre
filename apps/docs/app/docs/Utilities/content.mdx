...
Default merge-conflict buttons work even without `onMergeConflictAction`:
`UnresolvedFile` applies the resolution internally.

Provide `onMergeConflictAction` when you want controlled app state (for example,
to persist resolved contents, sync external stores, or trigger side effects). In
that case, `resolveMergeConflict` is the minimal pure update step.
