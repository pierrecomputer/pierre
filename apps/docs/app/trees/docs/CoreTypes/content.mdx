...
`FileTreeIconConfig` lets you swap built-in icons with your own SVG symbols.
Pass it via the `icons` option in `FileTreeOptions`.
...
Built-in icon names you can remap: `file-tree-icon-chevron`,
`file-tree-icon-file`, `file-tree-icon-dot`, `file-tree-icon-lock`.

<DocsCodeExample {...fileTreeIconConfigType} />
