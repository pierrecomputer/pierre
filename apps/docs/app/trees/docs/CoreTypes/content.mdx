...
`{ id, shadowHtml }` as `preloadedData` (React), or concatenate
`${payload.domOuterStart}${payload.shadowHtml}${payload.outerEnd}` when you need
to insert the full server-rendered container through DOM APIs. See [SSR](#ssr)
for usage.
...
| `flattenEmptyDirectories` | When `true`, single-child folder chains become one row (e.g. `build / assets / images` → `build · assets · images`). Default: `false`. [Live demo](../#flattening).                                                         |
...
| `gitStatus`               | Optional `GitStatusEntry[]` used to show Git-style file status (`added`, `modified`, `deleted`). Folders with changed descendants also receive a change indicator. [Live demo](../#path-colors).                            |
| `icons`                   | Optional built-in icon set selection or `FileTreeIconConfig` for semantic colors, CSS-themable palettes, and custom sprite overrides. [Live demo](../#custom-icons).                                                        |
