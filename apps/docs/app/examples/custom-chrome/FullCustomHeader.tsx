...
function SegmentedButtonGroup<T extends string>({
  ...
  const getButtonClasses = (isActive: boolean) => {
    ...
    return `flex h-6 w-6 items-center justify-center cursor-pointer rounded ${bg}`;
  };
...
export function FullCustomHeader({ prerenderedDiff }: FullCustomHeaderProps) {
  ...
  return (
    <div className="space-y-5">
      <div>
        <h2 className="text-2xl font-medium">Custom Header & Footer</h2>
        <p className="text-muted-foreground">
          Use <code>disableFileHeader: true</code> to completely replace the
