...
// Produces the two artifacts the path-store-powered demo needs to serve the
// AOSP workload without loading the full 141 MB fixture inside the Vercel
// function:
//
...
const previewOutputPath = resolvePath(
  repoRoot,
  'apps/docs/app/trees-dev/path-store-powered/aospPreview.ts'
);
const gzOutputDir = resolvePath(
  repoRoot,
  'apps/docs/public/path-store-powered'
);
const gzOutputPath = resolvePath(gzOutputDir, 'aosp-files.json.gz');

// Matches deriveExpandedPaths in pathStorePoweredWorkloadLoader.ts, but we
// precompute at artifact-build time so the client can simply read the result
// out of the gzip payload instead of walking 1.5 M paths after decompression.
...
// so the path-store-powered demo can server-render a representative viewport
